...
class nemo_output_factory(object):
    ...
    def get_times(self):
        ...
        period = None
        if funit == 'h':
            period = relativedelta(hours=+fnum)
        elif funit == 'd':
            period = relativedelta(days=+fnum)
        elif funit == 'm':
            period = relativedelta(months=+fnum)
        elif funit == 'y':
            period = relativedelta(years=+fnum)
        else:
    ...
    def write_variables(self, path_, prefix_, varg = None):
        vars_ = {} if varg is None else varg
        ...
        for v in vars_:
            ...
            if func:
                if dims == 2:
                    ...
                    raise Exception("Variables with dimensions ", dims, " are not supported")
            else:
                if dims == 2:

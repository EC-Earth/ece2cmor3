    ...
    for table, tasklist in taskdict.items():
        ...
        for task in tasklist:
            ...
            for yearfile in yearly_files:
                ...
                # Special treatment of Amon fco2nat (only when delivered by the EC-Earth3-CC configuration).
                # Adding (subtract) NEMO Omon fgco2 to the LPJG Amon fco2nat part: Amon fco2nat=Amon fco2nat_lpjg + Omon fgco2_nemo:
...
def find_timespan(lpjgfile):
    df = pd.read_csv(lpjgfile, sep=r'\s+', usecols=['Year'], dtype=np.int32)
...
def create_lpjg_netcdf(freq, inputfile, outname, outdims):
    ...
    df = pd.read_csv(inputfile, sep=r'\s+', index_col=idx_col, dtype=np.float64, compression='infer')
...
# This function is only used for getting monthly fgco2 from NEMO for Amon fco2nat (see https://github.com/EC-Earth/ece2cmor3/issues/798#issuecomment-1941233077):
def add_nemo_variable(task, ncfile, nemo_var_name, nemo_var_freq):
   ...
   #os.system("CDO_REMAP_NORM=destarea cdo -L invertlat -setmisstoc,0 -remapycon,"+remap_grid+" -selindexbox,2,361,2,292 -mul "+interm_file+" "+nemo_maskfile+" "+nemo_ofile)
    os.system("CDO_REMAP_NORM=destarea cdo -L           -setmisstoc,0 -remapycon,"+remap_grid+" -selindexbox,2,361,2,292 -mul "+interm_file+" "+nemo_maskfile+" "+nemo_ofile)
...
def find_nemo_file(varname, nemo_freq):
    # Find the nemo output folder (containing the varname at nemo_freq)for this leg (assumes it is in the runtime/output/nemo/??? folder)
    path_parts = splitpath(lpjg_path_)
    leg_str = path_parts[-1]
    nemo_path = path_parts[0]
    for d in range(1,len(path_parts)-2):
        nemo_path = os.path.join(nemo_path,path_parts[d])
    nemo_path = os.path.join(nemo_path,"nemo",leg_str)
...
# This function is only used for getting monthly fgco2 from NEMO for Amon fco2nat (see https://github.com/EC-Earth/ece2cmor3/issues/798#issuecomment-1941233077):
def splitpath(path):
    # Split an absolute path into all of its components.
    allparts = []
    if path[-1] == "/":
        path = path[:-1]       # Make sure there is no trailing slash
    while 1:

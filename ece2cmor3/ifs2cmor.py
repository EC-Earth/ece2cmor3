...
def get_output_freq(task):
    ...
    try:
        ...
        log.error(
            "Could not interpret environment variable ECE2CMOR3_IFS_NFRPOS with value %s as integer",
            str(env_val),
        )
        env_val = -1
    if env_val >= 0:
    ...
    if hasattr(task, cmor_task.filter_output_key):
        raise Exception(
            "Cannot determine post-processing frequency for %s, please provide it by setting the "
...
def initialize(path, expname, tableroot, refdate, tempdir=None, autofilter=True):
    global exp_name_, table_root_, ifs_gridpoint_files_, ifs_spectral_files_
    global ifs_init_spectral_file_, ifs_init_gridpoint_file_, temp_dir_
    global ref_date_, start_date_, auto_filter_
    ...
    gpfiles = {
        cmor_utils.get_ifs_date(f): os.path.join(path, f)
        for f in glob.glob1(path, "ICMGG" + file_pattern)
        if not f.endswith("+000000")
    }
    shfiles = {
        cmor_utils.get_ifs_date(f): os.path.join(path, f)
        for f in glob.glob1(path, "ICMSH" + file_pattern)
        if not f.endswith("+000000")
    }
    ...
    if any(shfiles) and any(gpfiles) and set(shfiles.keys()) != set(gpfiles.keys()):
        ...
        if not any(intersection):
            log.error(
                "Gridpoint files %s and spectral files %s correspond to different months, no overlap found...",
                str(list(gpfiles.values())),
                str(list(shfiles.values())),
            )
            ...
            log.warning(
                "Gridpoint files %s and spectral files %s correspond to different months, found overlapping "
                "dates %s",
                str(list(gpfiles.values())),
                str(list(shfiles.values())),
                str(intersection),
    ...
    if ifs_init_gridpoint_file_ is None:
        if any(ifs_gridpoint_files_.values()):
            ...
            log.error(
                "No gridpoint files found for experiment %s in directory %s, exiting initialization",
                exp_name_,
                path,
            )
            return False
    ...
    # Apply timeshift
    start_date_ = (
        datetime.combine(min(ifs_gridpoint_files_.keys()), datetime.min.time())
        - timeshift
    )
    ...
    if auto_filter_:
        ini_gpf = (
            None
            if ifs_init_gridpoint_file_ == list(ifs_gridpoint_files_.values())[0]
            else ifs_init_gridpoint_file_
        )
        grib_filter.initialize(
            ifs_gridpoint_files_,
            ifs_spectral_files_,
            temp_dir_,
            ini_gpfile=ini_gpf,
...
def validate_script_task(task):
    ...
    if script not in scripts:
        log.error(
            "Could not find post-processing script %s in ifspar.json, dismissing variable %s in table %s ",
            script,
            task.target.variable,
            task.target.table,
        )
    ...
    if scripts[script].get("src", None) is None:
        log.error(
            "Script source for %s has not been given, dismissing variable %s in table %s",
            script,
            task.target.variable,
            task.target.table,
        )
...
def execute(tasks, nthreads=1):
    ...
    script_tasks_no_filter = [
        t for t in script_tasks if validate_script_task(t) == "false"
    ]
    ...
    if ifs_init_gridpoint_file_.endswith("+000000"):
        ...
        tasks_to_filter = (
            mask_tasks + fx_tasks + extra_ps_tasks + regular_tasks + script_tasks_filter
        )
        tasks_no_filter = []
    np = nthreads
    # Launch no-filter scripts
    jobs = []
    tasks_per_script = cmor_utils.group(
        script_tasks_no_filter, lambda tsk: getattr(tsk, cmor_task.postproc_script_key)
    )
    for s, tasklist in list(tasks_per_script.items()):
        log.info(
            "Launching script %s to process variables %s",
            s,
            ",".join([t.target.variable + " in " + t.target.table for t in tasklist]),
        )
    ...
    if auto_filter_:
        ...
        for task in tasks_to_filter:
            if task.source.grid_id() == cmor_source.ifs_grid.point:
                setattr(
                    task,
                    cmor_task.filter_output_key,
                    list(ifs_gridpoint_files_.values()),
                )
                ...
                setattr(
                    task,
                    cmor_task.filter_output_key,
                    list(ifs_spectral_files_.values()),
                )
                tasks_todo.append(task)
            else:
                log.error(
    ...
    for task in list(
        set(tasks_todo).intersection(mask_tasks + req_ps_tasks + extra_ps_tasks)
    ):
...
def cmor_worker(task):
    if task.status in [
        cmor_task.status_failed,
        cmor_task.status_cmorized,
        cmor_task.status_finished,
    ]:
        return
    if validate_script_task(task) is not None:
        script = scripts[getattr(task, cmor_task.postproc_script_key)]["src"]
        log.info(
            "Post-processing variable %s for target variable %s using %s...",
            task.source.get_grib_code().var_id,
            task.target.variable,
            script,
        )
        ...
        ncfile = os.path.join(
            temp_dir_, task.target.variable + "_" + task.target.table + ".nc"
        )
        if os.path.isfile(ncfile):
            setattr(task, cmor_task.output_path_key, ncfile)
        else:
            log.error(
                "Output file %s of script %s could not be found... skipping cmorization of task",
                ncfile,
                script,
            )
            task.set_failed()
        ...
        log.info(
            "Post-processing variable %s for target variable %s...",
            task.source.get_grib_code().var_id,
            task.target.variable,
        )
        if task.status not in [
            cmor_task.status_postprocessing,
            cmor_task.status_postprocessed,
        ]:
            postproc.post_process(task, temp_dir_, do_post_process())
...
def script_worker(name, src, tasks):
    ...
    for task in tasks:
        ncfile = os.path.join(
            tmpdir, task.target.variable + "_" + task.target.table + ".nc"
        )
        if os.path.isfile(ncfile):
            setattr(task, cmor_task.output_path_key, ncfile)
        else:
...
def get_mask_tasks(tasks):
    ...
    for task in tasks:
        ...
        if msk:
            if msk not in masks:
                log.warning(
                    "Mask %s is not supported as an IFS mask, skipping masking", msk
                )
...
def read_mask(name, filepath):
    try:
        ...
        log.error(
            "Could not read netcdf file %s while reading mask %s, reason: %s",
            filepath,
            name,
            e.message,
        )
    ...
    try:
        ...
        varlist = [
            v for v in ncvars if str(getattr(ncvars[v], "code", None)) == codestr
        ]
        if len(varlist) == 0:
        ...
        if len(ncvar.shape) == 2:
            ...
            log.error(
                "After processing, the shape of the mask variable is %s which cannot be applied to time slices",
                str(ncvar.shape),
            )
            return
    ...
    for task in tasks:
        for key in [cmor_task.filter_output_key, cmor_task.output_path_key]:
            ...
            for dpath in data_paths:
                ...
                if (
                    dp
                    not in list(ifs_spectral_files_.values())
                    + list(ifs_gridpoint_files_.values())
                    and dp in tmp_files
                ):
                    try:
...
def filter_tasks(tasks):
    ...
    for task in tasks:
        ...
        if (
            (haslat and haslon)
            or (haslat and not haslon)
            or (not haslat and not haslon)
        ):
            result.append(task)
        else:
            # TODO: Support zonal variables
            log.error(
                "Variable %s has unsupported combination of dimensions %s and will be skipped.",
                task.target.variable,
                tgtdims,
            )
...
def get_sp_tasks(tasks):
    ...
    for freq, task_group in tasks_by_freq.items():
        ...
        if not any(tasks3d):
            continue
        surf_pressure_tasks = [
            t for t in task_group if t.source.get_grib_code() == surface_pressure
        ]
        ...
        if any(surf_pressure_tasks):
            ...
            surf_pressure_task = cmor_task.cmor_task(
                source, cmor_target.cmor_target("ps", tasks3d[0].target.table)
            )
            ...
            setattr(
                surf_pressure_task.target, cmor_target.dims_key, "latitude longitude"
            )
...
def define_cmor_axes(task):
    ...
    if use_2d_grid() and has_lats and has_lons:
        if global_grid_id == -1:
            ...
            global_grid_id = create_grid_from_file(
        ...
        if (
            grid_ids is None
            or (has_lons and grid_ids[0] is None)
            or (has_lats and grid_ids[1] is None)
        ):
            ...
            if downloaded_file is not None:
                ...
                grid_ids = create_grid_from_file(
                    getattr(task, cmor_task.output_path_key)
                )
    ...
    try:
        ...
        log.error(
            "CMOR failed to load table %s, the following variable will be skipped: %s. Reason: %s",
            task.target.table,
            task.target.variable,
            e.message,
        )
...
def execute_netcdf_task(task):
    ...
    surf_pressure_path = (
        getattr(surf_pressure_task, "path", None) if surf_pressure_task else None
    )
    ...
    try:
        ...
        if len(getattr(task.target, "positive", "")) > 0:
            var_id = cmor.variable(
                table_entry=str(task.target.variable),
                units=str(unit),
                axis_ids=axes,
                positive="up"
                if getattr(task, cmor_task.conversion_key, None) == "vol2fluxup"
                else "down",
            )
        else:
            var_id = cmor.variable(
                table_entry=str(task.target.variable), units=str(unit), axis_ids=axes
            )
        ...
        cmor_utils.netcdf2cmor(
            ...
            swaplatlon=False,
            fliplat=(not hasattr(task, "download")),
            mask=mask_array,
            missval=missval,
            time_selection=time_selection,
            force_fx=(cmor_target.get_freq(task.target) == 0),
        )
        cmor.close(var_id)
        task.next_state()
        if store_var:
...
def create_time_axes(task):
    ...
    if len(time_dims) > 1:
        log.error(
            "Skipping variable %s in table %s with dimensions %s with multiple time dimensions.",
            task.target.variable,
            task.target.table,
            tgtdims,
        )
...
def create_depth_axes(task):
    ...
    if len(z_dims) > 1:
        log.error(
            "Skipping variable %s in table %s with dimensions %s with multiple z-directions.",
            task.target.variable,
            task.target.table,
            tgtdims,
        )
    ...
    if key in depth_axis_ids:
        ...
        log.info(
            "Creating soil depth axis 1 using variable %s...", task.target.variable
        )
        axisid = create_soil_depth_axis(z_dim)
        depth_axis_ids[key] = axisid
        setattr(task, "z_axis_id", axisid)
        ...
        if len(levels) == 0:
            log.warning(
                "Skipping axis %s in table %s with no levels", z_dim, task.target.table
            )
            return
        else:
            ...
            if axis.get("must_have_bounds", "no") == "yes":
                ...
                if not bounds_list:
                    bounds_list = [
                        float(x) for x in axis.get("bounds_values", []).split()
                    ]
                if len(bounds_list) == 2 * n:
                    bounds_array = numpy.empty([n, 2])
                    for i in range(n):
                        bounds_array[i, 0], bounds_array[i, 1] = (
                            bounds_list[2 * i],
                            bounds_list[2 * i + 1],
                        )
                    axisid = cmor.axis(
                        table_entry=str(z_dim),
                        coord_vals=values,
                        units=unit,
                    ...
                    log.error(
                        "Failed to retrieve bounds for vertical axis %s", str(z_dim)
                    )
                    axisid = cmor.axis(
                        table_entry=str(z_dim), coord_vals=values, units=unit
                    )
            else:
                axisid = cmor.axis(
                    table_entry=str(z_dim), coord_vals=values, units=unit
        ...
        log.error(
            "Vertical dimension %s for variable %s not found in header of table %s",
            z_dim,
            task.target.variable,
            task.target.table,
        )
...
def create_hybrid_level_axis(task):
    ...
    try:
        ...
        storewith = cmor.zfactor(
            zaxis_id=axisid, zfactor_name=zfactor_name, axis_ids=axes, units="Pa"
        )
...
def create_soil_depth_axis(name):
    ...
    return cmor.axis(
        table_entry=name, coord_vals=values, cell_bounds=bounds, units="cm"
    )
...
def create_time_axis(freq, path, name, has_bounds):
    ...
    if has_bounds:
        ...
        rounded_times = [
            (cmor_utils.get_rounded_time(freq, time)) for time in date_times
        ]
        dt_low = rounded_times
        dt_up = rounded_times[1:] + [
            cmor_utils.get_rounded_time(freq, date_times[-1], 1)
        ]
        bounds[:, 0], units = cmor_utils.date2num(
            [t - timeshift for t in dt_low], ref_date_
        )
        bounds[:, 1], units = cmor_utils.date2num(
            [t - timeshift for t in dt_up], ref_date_
        )
        times = bounds[:, 0] + (bounds[:, 1] - bounds[:, 0]) / 2
        return (
            cmor.axis(
                table_entry=str(name), units=units, coord_vals=times, cell_bounds=bounds
            ),
            dt_low,
            dt_up,
        )
    ...
    if date_times[0] >= start_date_ + step:
        ...
        log.warning(
            "The file %s seems to be missing %d time stamps at the beginning, these will be added",
            path,
            len(extra_dates),
        )
    ...
    if date_times[0] < start_date_:
        ...
        log.warning(
            "The file %s seems to be containing %d too many time stamps at the beginning, these will be "
            "removed",
            path,
            len([t for t in date_times if t >= start_date_]),
        )
    times, units = cmor_utils.date2num([t - timeshift for t in date_times], ref_date_)
    return (
        cmor.axis(table_entry=str(name), units=units, coord_vals=times),
        date_times,
        date_times,
    )
...
def get_sp_var(ncpath):
    ...
    try:
        ...
        log.error(
            "Could not read netcdf file %s for surface pressure, reason: %s",
            ncpath,
            e.message,
        )
        return None
...
def copy_grid_from_file(filepath):
    ...
    lats = cmor.axis(
        ...
        coord_vals=ds.variables["lat"][...],
        cell_bounds=ds.variables["lat_bnds"][...],
        units="degrees_north",
    )
    lons = cmor.axis(
        table_entry="longitude",
        coord_vals=ds.variables["lon"][...],
        cell_bounds=ds.variables["lon_bnds"][...],
        units="degrees_east",
    )
    ds.close()
    return lats, lons
...
def create_gauss_grid(xvals, yvals):
    ...
    if use_2d_grid() and nx > 1 and ny > 1:
        ...
        i_index_id = cmor.axis(
            table_entry="i_index",
            units="1",
            coord_vals=numpy.array(list(range(1, nx + 1))),
        )
        j_index_id = cmor.axis(
            table_entry="j_index",
            units="1",
            coord_vals=numpy.array(list(range(1, ny + 1))),
        )
        lon_arr = numpy.tile(xvals, (ny, 1))
        lat_arr = numpy.tile(yvals[::-1], (nx, 1)).transpose()
        return cmor.grid(
            axis_ids=[j_index_id, i_index_id],
            latitude=lat_arr,
            longitude=lon_arr,
        ...
        lats = (
            cmor.axis(
                table_entry="latitude",
                coord_vals=yvals[::-1],
                cell_bounds=get_lat_mids(yvals)[::-1],
                units="degrees_north",
            )
            if (ny > 1)
            else None
        ...
        lons = (
            cmor.axis(
                table_entry="longitude",
                coord_vals=xvals,
                cell_bounds=get_lon_mids(xvals),
                units="degrees_east",
            )
            if (nx > 1)
            else None
...
def download_sftlf(task):
    ...
    fullpath = os.path.join(
        os.path.dirname(__file__), "resources", "b2share-data", fname
    )
    if not os.path.isfile(fullpath):
        if not cmor_utils.get_from_b2share(fname, fullpath):
            log.fatal(
                "The file %s could not be downloaded, please install manually at %s",
                fname,
                fullpath,
            )
            return None

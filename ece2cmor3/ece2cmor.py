...
def main(args=None):
    ...
    parser.add_argument(
        ...
        metavar="DIR",
        type=str,
        help="EC-Earth data directory, i.e. for a given component, " "for a given leg",
    )
    parser.add_argument(
        "--exp", metavar="EXPID", type=str, default="ECE3", help="Experiment prefix"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory, by default the " "metadata 'outpath' entry",
    )
    cmor_utils.ScriptUtils.add_model_exclusive_options(parser, "ece2cmor")
    parser.add_argument(
        "--ececonf",
        metavar="|".join(list(components.ece_configs.keys())),
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Number of parallel tasks (only relevant for " "IFS cmorization",
    )
    parser.add_argument(
        "--log", action="store_true", default=False, help="Write to log file"
    )
    parser.add_argument(
        "--flatdir",
        action="store_true",
        default=False,
        help="Do not create sub-directories in " "output folder",
    )
    parser.add_argument(
        "--tabledir",
        metavar="DIR",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Prevent loading atmospheric " "model-level variables",
    )
    parser.add_argument(
        "-V",
        "--version",
    ...
    parser.add_argument(
        "--ncdo", metavar="N", type=int, default=4, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--nomask", action="store_true", default=False, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--nofilter", action="store_true", default=False, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--atm",
        action="store_true",
    ...
    parser.add_argument(
        "--conf", action="store_true", help="Deprecated! Use --meta instead"
    )
    parser.add_argument(
        "--vars", action="store_true", help="Deprecated! Use --varlist instead"
    )
    ...
    if getattr(args, "log", False):
        ...
        fname = "-".join(
            [args.exp] + dirs[-2:] + [time.strftime("%Y%m%d%H%M%S", time.gmtime())]
        )
        ...
        logging.basicConfig(
            level=logging.DEBUG, format=logformat, datefmt=logdateformat
        )
    ...
    refdate = datetime.datetime.combine(
        dateutil.parser.parse(args.refd), datetime.datetime.min.time()
    )

    if "ifs" in active_components:
        ece2cmorlib.perform_ifs_tasks(
            args.datadir,
            args.exp,
            refdate=refdate,
            tempdir=args.tmpdir,

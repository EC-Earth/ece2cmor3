...
def add_level_operators(cdo,task):
    ...
    if(len(levels) == 0):
        ...
        if(val): levels = [val]
    leveltypes = [grib_file.hybrid_level_code, grib_file.pressure_level_code, grib_file.height_level_code]
    ...
    if(oname == "air_pressure"):
        add_zaxis_operators(cdo, task, leveltypes, levels, cdoapi.cdo_command.pressure, grib_file.pressure_level_code)
    elif(oname in ["height","altitude"]):
        add_zaxis_operators(cdo, task, leveltypes, levels, cdoapi.cdo_command.height, grib_file.height_level_code)

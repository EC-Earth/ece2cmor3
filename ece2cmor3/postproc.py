...
def post_process(task, path, output_freq, grid_descr=None, cdo_threads=1):
...
def apply_command(command, task_list, cdo_threads, base_path=None):
    global log
    if not task_list:
...
def add_time_operators(cdo, task, source_freq):
    ...
    if freq == "mon":
        ...
        add_high_freq_operator(cdo, source_freq, 6, operators[0], task)
    elif freq in ["3hr", "3hrPt"] and len(operators) == 1:
        add_high_freq_operator(cdo, source_freq, 3, operators[0], task)
...
def add_high_freq_operator(cdo_command, source_freq, target_freq, operator, task):
    ...
    if operator == "point":
        ...
        if not all([c for c in task.source.get_root_codes() if c in aggregators[operator][0]]):
            source_freq = getattr(task, cmor_task.output_frequency_key, source_freq)

...
extra_axes = {
    ...
    "iceband": {
        ...
        "ncbnds": [0.0, 0.454, 1.129, 2.141, 3.671, 99.0],
    },
}
...
def initialize(path, expname, tableroot, refdate, testmode=False):
    ...
    if not os.path.isfile(bathy_file_):
        ...
        bathy_file_ = os.environ.get(
            "ECE2CMOR3_NEMO_BATHY_METER", os.path.join(expdir, "bathy_meter.nc")
        )
    if not os.path.isfile(bathy_file_):
        log.warning(
            "Nemo bathymetry file %s does not exist...variable deptho in Ofx will be dismissed "
    ...
    if not os.path.isfile(basin_file_):
        # Look in env or ec-earth run directory
        basin_file_ = os.environ.get(
            "ECE2CMOR3_NEMO_SUBBASINS", os.path.join(expdir, "subbasins.nc")
        )
    if not os.path.isfile(basin_file_):
        log.warning(
            "Nemo subbasin file %s does not exist...variable basin in Ofx will be dismissed "
...
def execute(tasks):
    ...
    task_groups = cmor_utils.group(
        tasks, lambda tsk1: getattr(tsk1, cmor_task.output_path_key, None)
    )
    for filename, task_group in task_groups.items():
        ...
        for table, task_list in task_sub_groups.items():
            log.info(
                "Start cmorization of %s in table %s"
                % (",".join([t.target.variable for t in task_list]), table)
            )
            try:
                ...
                log.error(
                    ...
                    % (
                        table,
                        ",".join([tsk3.target.variable for tsk3 in task_list]),
                        e.message,
                    )
                )
                continue
            if table not in time_axes_:
                log.info(
                    "Creating time axes for table %s from data in %s..."
                    % (table, filename)
                )
            create_time_axes(dataset, task_list, table)
            if table not in depth_axes_:
                log.info(
                    "Creating depth axes for table %s from data in %s ..."
                    % (table, filename)
                )
            create_depth_axes(dataset, task_list, table)
            if table not in type_axes_:
                log.info(
                    "Creating extra axes for table %s from data in %s ..."
                    % (table, filename)
                )
    ...
    for task in tasks:
        if (task.target.table, task.target.variable) == ("Ofx", "deptho"):
            if bathy_file_ is None:
                log.error(
                    "Could not use bathymetry file for variable deptho in table Ofx: task skipped."
                )
                task.set_failed()
            else:
        ...
        if (task.target.table, task.target.variable) == ("Ofx", "basin"):
            if basin_file_ is None:
                log.error(
                    "Could not use subbasin file for variable basin in table Ofx: task skipped."
                )
                task.set_failed()
            else:
        ...
        if len(results) == 0:
            log.error(
                "Variable {:20} in table {:10} was not found in the NEMO output files: task skipped.".format(
                    task.source.variable(), task.target.table
                )
            )
            task.set_failed()
            continue
        if len(results) > 1:
            log.error(
                "Variable %s needed for %s in table %s was found in multiple NEMO output files %s... "
                "dismissing task"
                % (
...
def create_basins(target, dataset):
    meanings = {
        "atlmsk": "atlantic_ocean",
        "indmsk": "indian_ocean",
        "pacmsk": "pacific_ocean",
    }
...
def execute_netcdf_task(dataset, task):
    ...
    type_axes = [
        ...
        for dim in list(type_axes_.get(task.target.table, {}).keys())
    ...
    if task.target.variable == "basin":
        ...
        missval = getattr(
            ncvar, "missing_value", getattr(ncvar, "_FillValue", numpy.nan)
        )
    ...
    log.info(
        "Cmorizing variable {:20} in table {:7} in file {}".format(
...
def create_cmor_variable(task, srcvar, ncvar, axes):
    ...
    if hasattr(task.target, "positive") and len(task.target.positive) != 0:
        return cmor.variable(
            ...
            positive=getattr(task.target, "positive"),
        )
    else:
        return cmor.variable(
            table_entry=str(task.target.variable),
...
def create_depth_axes(ds, tasks, table):
    ...
    if table not in depth_axes_:
        depth_axes_[table] = {}
    log.info(
        "Creating depth axes for table %s using file %s..." % (table, ds.filepath())
    )
    table_depth_axes = depth_axes_[table]
    other_nc_axes = ["time_counter", "x", "y"] + [
        extra_axes[k]["ncdim"] for k in list(extra_axes.keys())
    ]
    for task in tasks:
        z_axes = []
        if task.source.variable() in ds.variables:
            z_axes = [
                d
                for d in ds.variables[task.source.variable()].dimensions
                if d not in other_nc_axes
            ]
        z_axis_ids = []
        for z_axis in z_axes:
            if z_axis not in ds.variables:
                log.error(
                    "Cannot find variable %s in %s for vertical axis construction"
                    % (z_axis, ds.filepath())
                )
                continue
            ...
            axis_type = (
                "half"
                if cmor_target.get_z_axis(task.target)[0] == "olevhalf"
                else "full"
            )
            ...
            if key in table_depth_axes:
                ...
                if depth_bounds is None:
                    log.warning(
                        "No depth bounds found in file %s, taking midpoints"
                        % (ds.filepath())
                    )
                ...
                if len(units) == 0:
                    log.warning(
                        "Assigning unit meters to depth coordinate %s without units"
                        % entry
                    )
                    units = "m"
                b = depth_bounds[:, :]
                b[b < 0] = 0
                z_axis_id = cmor.axis(
                    table_entry=entry, units=units, coord_vals=zvar[:], cell_bounds=b
                )
...
def create_time_axes(ds, tasks, table):
    ...
    if table not in time_axes_:
        time_axes_[table] = {}
    log.info(
        "Creating time axis for table %s using file %s..." % (table, ds.filepath())
    )
    ...
    for task in tasks:
        ...
        for time_dim in [d for d in list(set(tgtdims.split())) if d.startswith("time")]:
            if time_dim in table_time_axes:
                ...
                nc_operator = getattr(
                    ds.variables[task.source.variable()], "online_operation", "instant"
                )
                if (
                    time_operator[0] in ["point", "instant"]
                    and nc_operator != "instant"
                ):
                    log.warning(
                        "Cmorizing variable %s with online operation attribute %s in %s to %s with time "
                        "operation %s"
                        % (
                            task.source.variable(),
                            nc_operator,
                ...
                if times is None:
                    log.error(
                        "Failed to read time axis information from file %s, skipping variable %s in table %s"
                        % (ds.filepath(), task.target.variable, task.target.table)
                    )
                    task.set_failed()
                    continue
                tstamps, tunits = cmor_utils.num2num(
                    times, ref_date_, units, calendar, timeshift
                )
                if calendar != "proleptic_gregorian":
                    cmor.set_cur_dataset_attribute("calendar", calendar)
                if time_bounds is None:
                    tid = cmor.axis(
                        table_entry=str(time_dim), units=tunits, coord_vals=tstamps
                    )
                else:
                    tbounds, tbndunits = cmor_utils.num2num(
                        time_bounds, ref_date_, units, calendar, timeshift
                    )
                    tid = cmor.axis(
                        table_entry=str(time_dim),
                        units=tunits,
                        coord_vals=tstamps,
                        cell_bounds=tbounds,
                    )
...
def read_times(ds, task):
    ...
    if cmor_target.is_instantaneous(task.target):
        ...
        if ncvar is not None:
            vals, units, calendar = (
                ncvar[:],
                getattr(ncvar, "units", None),
                getattr(ncvar, "calendar", None),
            )
        else:
            log.warning(
                "Could not find time_instant variable in %s, looking for generic time..."
                % ds.filepath()
            )
            for varname, ncvar in list(ds.variables.items()):
                if getattr(ncvar, "standard_name", "").lower() == "time":
                    log.warning(
                        "Found variable %s for instant time variable in file %s"
                        % (varname, ds.filepath())
                    )
                    vals, units, calendar = (
                        ncvar[:],
            ...
            if vals is None:
                log.error(
                    "Could not find time variable in %s for %s... giving up"
                    % (ds.filepath(), str(task.target))
                )
    else:
        ncvar = ds.variables.get("time_centered", None)
        if ncvar is not None:
            vals, bndvals, units, calendar = (
                ncvar[:],
                get_time_bounds(ncvar),
                getattr(ncvar, "units", None),
                getattr(ncvar, "calendar", None),
            )
        else:
            log.warning(
                "Could not find time_centered variable in %s, looking for generic time..."
                % ds.filepath()
            )
            for varname, ncvar in list(ds.variables.items()):
                if getattr(ncvar, "standard_name", "").lower() == "time":
...
def create_type_axes(ds, tasks, table):
    ...
    if table not in type_axes_:
        type_axes_[table] = {}
    log.info(
        "Creating extra axes for table %s using file %s..." % (table, ds.filepath())
    )
    table_type_axes = type_axes_[table]
    for task in tasks:
        tgtdims = set(getattr(task.target, cmor_target.dims_key).split()).intersection(
            list(extra_axes.keys())
        )
        for dim in tgtdims:
            if dim in table_type_axes:
                ...
                if nc_dim_name in ds.dimensions:
                    ncdim, ncvals = ds.dimensions[nc_dim_name], axisinfo.get(
                        "ncvals", []
                    )
                    if len(ncdim) == len(ncvals):
                        ...
                        if any(ncvals):
                            log.error(
                                "Ece2cmor values for extra axis %s, %s, do not match dimension %s length %d found"
                                " in file %s, taking values found in file"
                                % (
                                    dim,
                                    str(ncvals),
                                    nc_dim_name,
                        ...
                        ncvars = [
                            ...
                            if list(ds.variables[v].dimensions) == [ncdim]
                        ]
                        axis_values, axis_unit = list(range(len(ncdim))), "1"
                        if any(ncvars):
                            if len(ncvars) > 1:
                                log.warning(
                                    "Multiple axis variables found for dimension %s in file %s, choosing %s"
                                    % (nc_dim_name, ds.filepath(), ncvars[0])
                                )
                            axis_values, axis_unit = list(ncvars[0][:]), getattr(
                                ncvars[0], "units", None
                            )
                else:
                    log.error(
                        "Dimension %s could not be found in file %s, inserting using length-one dimension "
                ...
                if "ncbnds" in axisinfo:
                    ...
                    if len(bndlist) - 1 != len(axis_values):
                        log.error(
                            "Length of axis bounds %d does not correspond to axis coordinates %s"
                    ...
                    axis_id = cmor.axis(
                        table_entry=dim,
                        coord_vals=axis_values,
                        units=axis_unit,
                        cell_bounds=bnds,
                    )
                else:
                    axis_id = cmor.axis(
                        table_entry=dim, coord_vals=axis_values, units=axis_unit
                    )
...
def select_freq_files(freq, varname):
    ...
    #    nemo_freq = "1m"   # check
        ...
        log.error(
            "Could not associate cmor frequency {:7} with a "
        ...
        if mask is not None and mask not in list(nemo_masks_.keys()):
            for nemo_file in nemo_files_:
                ...
                if maskvar is not None:
                    ...
                    if len(dims) == 2:
                        nemo_masks_[mask] = numpy.logical_not(
                            numpy.ma.getmask(maskvar[...])
                        )
                    elif len(dims) == 3:
                        nemo_masks_[mask] = numpy.logical_not(
                            numpy.ma.getmask(maskvar[0, ...])
                        )
                    else:
                        log.error(
                            "Could not create mask %s from nc variable with %d dimensions"
                            % (mask, len(dims))
                        )
...
def read_grid(ncfile):
    ...
    try:
        ...
        name = getattr(
            ds.variables["nav_lon"], "nav_model", cmor_utils.get_nemo_grid(ncfile)
        )
        if name == "scalar":
            return None
...
def write_grid(grid, tasks):
    ...
    if ny == 1:
        ...
        if grid.name not in grid_ids_:
            ...
            grid_id = cmor.grid(
                axis_ids=[i_index_id, j_index_id],
                latitude=grid.lats,
                longitude=grid.lons,
    ...
    def __init__(self, name_, lons_, lats_):
        ...
        if input_lats.shape[1] == 1:
            if input_lats.shape[0] > 2 and input_lats[-1, 0] <= input_lats[-2, 0]:
                input_lats[-1, 0] = input_lats[-2, 0] + (
                    input_lats[-2, 0] - input_lats[-3, 0]
                )
        ...
        if input_lats.shape[0] == 1 or input_lats.shape[1] == 1 or gridtype is None:
            ...
            log.info(
                "Using fallback to the old midpoint calculation method for grid %s with shape %s"
            ...
            self.vertex_lons, self.vertex_lats = load_vertices_from_file(
                gridtype, input_lats.shape
            )
            if self.vertex_lons is None or self.vertex_lats is None:
                if test_mode_:
                    ...
                    raise Exception(
                        "Vertices could not be loaded from ece2cmor3 nc files"
                    )

...
def load_vertices_from_file(gridtype, shape):
    ...
    if not os.path.isfile(fullpath):
        if not cmor_utils.get_from_b2share(file_name, fullpath):
            ...
            return None, None
    nemo_vertices_file_name = os.path.join(
        "ece2cmor3/resources/b2share-data/", fullpath
    )
    nemo_vertices_netcdf_file = netCDF4.Dataset(nemo_vertices_file_name, "r")
    lon_vertices_raw = numpy.array(
        nemo_vertices_netcdf_file.variables["vertices_longitude"][...], copy=True
    )
    lat_vertices = numpy.array(
        nemo_vertices_netcdf_file.variables["vertices_latitude"][...], copy=True
    )

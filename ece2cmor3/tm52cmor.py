...
extra_axes = {
    "lambda550nm": {"ncdim": "lambda550nm", "ncunits": "nm", "ncvals": [550.0]}
}
...
def check_freqid(task):
    ...
    if task.target.frequency == "monC":
        if task.target.table == "Amon" and (
            task.target.variable == "pfull" or task.target.variable == "phalf"
        ):
            task.set_failed()
            log.info(
                "Variable %s in table %s will be produced by IFS"
                % (task.target.variable, task.target.table)
            )
            return False, None
    elif task.target.frequency in ignore_frequency:
        log.info(
            "frequency %s ignored, no data prduced at this frequency"
...
def execute(tasks):
    ...
    for task in tasks:
        ...
        if task.target.frequency == "fx":
            ...
            if "Clim" in task.target.variable:
                ...
                task.set_failed()
                continue
            elif task.target.table == "Amon" and (
                task.target.variable == "pfull" or task.target.variable == "phalf"
            ):
                task.set_failed()
                log.info(
        ...
        if not success:
            task.set_failed()
            log.info(
                "Frequency %s for task %s not available.",
                task.target.frequency,
        ...
        for fstr in tm5_files_:
            ...
            if (
                os.path.basename(fstr).startswith(task.source.variable() + "_")
                and freqid + "_" in fstr
            ):
                fname = fstr
                if getattr(task, cmor_task.output_path_key) == None:
                    ...
                    log.critical(
                        "Second file with same frequency and name. Currently supporting only one year per directory."
                    )
                    log.critical(fstr)
                    exit(" Exiting ece2cmor.")
            if not os.path.exists(fstr):
                log.info(
                    "No path found for variable %s from TM5" % (task.target.variable)
                )
                task.set_failed()
                continue
    ...
    for table, tasklist in taskdict.items():
        try:
            log.info(
                "Loading CMOR table %s to process %d variables..."
                % (table, len(tasklist))
            )
            ...
            log.error(
                "ERR -6: CMOR failed to load table %s, skipping variables %s. Reason: %s"
                % (
                    table,
        ...
        if table == "3hr":
            ...
            log.error(
                "ERR -6: Skipping variable %s not implemented" % (task.target.variable)
            )
            continue
            # postprocess data to zonal mean and plev39, or do it before this point
        if table == "Eday":
            log.info(
                "Table Eday not supported for variable %s " % (task.target.variable)
            )
        ...
        for task in tasklist:
            ...
            # 2D grid
            if (
                task.target.variable == "ch4Clim"
                or task.target.variable == "ch4globalClim"
                or task.target.variable == "o3Clim"
            ):
                log.error(
                    "ERR -8: Task for %s is not produced in any of the simulations with TM5."
                    % task.target.variable
                )
                task.set_failed()
                continue
            ...
            if taskmask[task]:
                ...
                if task.status not in [cmor_task.status_failed]:
                    log.info(
                        "Cmorizing source variable %s to target variable %s from file %s."
                        % (task.source.variable(), task.target.variable, ncf)
                    )
                    execute_netcdf_task(task, tab_id)
                    if task.status < 0:
                        if task.target.variable == "cdnc":
                            log.error(
                                "ERR -10: Cmorizing failed for %s, but variable is produced by IFS."
                                % (task.target.variable)
                            )
                        elif task.target.variable == "o3Clim":
                            log.error(
                                "ERR -11: Cmorizing failed for %s, check tm5par.json since source will be o3 instead of %s."
                                % (task.target.variable, task.source.variable())
                            )
                        elif task.target.variable == "phalf":
                            log.error(
                                "ERR -11: Cmorizing failed for %s,  but variable is produced by IFS."
                                % (task.target.variable)
                            )
                        elif (
                            task.target.variable == "ch4Clim"
                            or task.target.variable == "ch4global"
                            or task.target.variable == "ch4globalClim"
                        ):
                            log.error(
                                "ERR -12: Cmorizing failed for %s, check tm5par.json since source will be ch4 instead of %s."
                                % (task.target.variable, task.source.variable())
                            )
                        else:
                            log.error(
                                "ERR -13: Cmorizing failed for %s"
                                % (task.target.variable)
                            )
                    else:
                        taskmask[task] = True
                    ...
                    log.info(
                        "Skipping variable %s for unknown reason..."
                        % (task.source.variable())
                    )
        for task, executed in taskmask.items():
            if not executed:
                log.error(
                    "ERR -14: The source variable %s of target %s in  table %s failed to cmorize"
...
def execute_netcdf_task(task, tableid):
    ...
    if not filepath:
        log.error(
            "ERR -15: Could not find file containing data for variable %s in table %s"
            % (task.target.variable, task.target.table)
        )
        task.set_failed()
        return
    ...
    if task.target.dims >= 3:
        ...
        log.error(
            "ERR -19: unsupported dimensions %s for variable"
            % (task.target.dims, task.target.variable)
        )
        exit()
    ...
    for key in type_axes_:
        ...
        if key[0] == task.target.table and key[1] in getattr(
            task.target, cmor_target.dims_key
        ):
    ...
    try:
        ...
        log.error(
            "ERR -20: Could not read netcdf file %s while cmorizing variable %s in table %s. Cause: %s"
    ...
    if interpolate_to_pressure:
        psdata = get_ps_var(
            getattr(getattr(task, "ps_task", None), cmor_task.output_path_key, None)
        )
        pressure_levels = getattr(task, "pressure_levels")
        ncvar = interpolate_plev(
            pressure_levels, dataset, psdata, task.source.variable()
        )
    ...
    # handle global means
        ...
        if task.target.variable == "co2mass":
            ...
            vals = numpy.sum(
                (vals * areacella_[numpy.newaxis, :, :]), axis=(1, 2)
            ) / numpy.sum(areacella_)
        ncvar = vals.copy()
    # handle normal case
    ...
    for task in tasks:
        ...
        for time_dim in [d for d in list(set(tgtdims.split())) if d.startswith("time")]:
            ...
            if key in time_axes:
                ...
                log.info(
                    "Creating time axis using variable %s..." % task.target.variable
                )
                tid = create_time_axis(
...
def create_time_axis(path, name, has_bounds):
    ...
    if len(vals) == 0 or units == None:
        log.error(
            "ERR -22: No time values or units could be read from tm5 output files %s"
            % str(path)
        )
        return -1
...
def create_type_axes(task):
    ...
    for dim in tgtdims:
        if dim == "lambda550nm":
            ...
            ax_id = cmor.axis(
                table_entry="lambda550nm", units=ncunits, coord_vals=ncvals
            )
...
def create_depth_axes(task):
    ...
    if len(zdims) > 1:
        log.error(
            "ERR -23: Skipping variable %s in table %s with dimensions %s with multiple z-directions."
    ...
    if key not in depth_axis_ids:
        log.info(
            "Creating vertical axis %s for table %s..." % (zdim, task.target.table)
        )
    ...
    if key in depth_axis_ids:
        ...
        log.info(
            "Creating model full level axis for variable %s..." % task.target.variable
        )
        ...
        log.info(
            "Creating model half level axis for variable %s..." % task.target.variable
        )
        ...
        log.critical(
            "Z-dimension %s not found for variable %s..." % (zdim, task.target.variable)
        )
        return False
...
def create_hybrid_level_axis(task, leveltype="alevel"):
    ...
    try:
        ...
        if leveltype == "alevel":
            ...
            axisid = cmor.axis(
                table_entry="alternate_hybrid_sigma_half", coord_vals=hci, units="1"
            )
        ...
        if task.target.table not in zfactor_ids:
            storewith = cmor.zfactor(
                zaxis_id=axisid, zfactor_name=psvarname, axis_ids=axes, units="Pa"
            )
...
def get_ps_var(ncpath):
    ...
    try:
        ...
        log.error(
            "ERR -5: Could not read netcdf file %s for surface pressure, reason: %s"
            % (ncpath, e.message)
        )
        return None
...
def get_ps_tasks(tasks):
    ...
    for freq, task_group in tasks_by_freq.items():
        tasks3d = [
            ...
            for t in task_group
            if (
                "alevel" in getattr(t.target, cmor_target.dims_key).split()
                or "plev19" in getattr(t.target, cmor_target.dims_key).split()
                or "alevhalf" in getattr(t.target, cmor_target.dims_key).split()
                or "plev39" in getattr(t.target, cmor_target.dims_key).split()
            )
        ]
        if not any(tasks3d):
            continue

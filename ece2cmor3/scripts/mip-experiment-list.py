...
for mip in dq.coll["mip"].items:
    ...
    print(
        "{:3} {:15} {}".format(
            ...
            mip.label,
            str(mip)
            .replace("Item <1.1 Model Intercomparison Project>: [", "")
            .replace("]", "")
            .replace(mip.label, "")
            .replace(" - set of", "DECK - set of"),
        )
    )
    ...
    for u in dq.inx.iref_by_sect[mip.uid].a["experiment"]:
        ...
        if mip.label != ex.mip:
            print(
                " The mip.label and ex.mip are different: {:15} versus {:15}".format(
                    mip.label, ex.mip
                )
            )
        mip_list_file.write(
            "{:12} {:28} tier-{:1}  {}".format(ex.mip, ex.label, ex.tier[0], ex.title)
            + "\n"
        )
...
print(
    "Number of MIPs is: {:3} and the number of MIP experiments is: {}".format(
        mip_counter, experiment_counter
    )
)
...
for mip in dq.coll["mip"].items:
    if mip.label in ec_earth_mips:
        ...
        for u in dq.inx.iref_by_sect[mip.uid].a["experiment"]:
            ...
            ex = dq.inx.uid[u]
            mip_list_file.write(
                " {:28} tier-{:1}  {}".format(ex.label, ex.tier[0], ex.title) + "\n"
            )

...
for k, v in datastore["CV"]["experiment_id"].items():
    ...
    if k != v["experiment_id"]:
        print(
            " IDs differ: {:30} {:30} {:30}".format(
                v["activity_id"][0], v["experiment_id"], k
            )
        )
    if len(v["activity_id"][:]) != 1:
        print(
            " The activity_id has multiple entries: {:30} {:30}".format(
                v["activity_id"], k
            )
        )
    ...
    for counter in range(n_activity_id_list):
        # print ' {:3} {:30} {:30} {:30}'.format(counter, k, v["activity_id"][0], activity_id_list[counter])
        code_parent_check_file.write(
            ' if [ "$mip" = {:30} ] && [ "$experiment" = {:30} ]; then declare -a parent_info=({:30} {:30} {:30} {:30} {:30} {:30} {:200} {}); fi\n'.format(
                "'" + activity_id_list[counter] + "'",
                "'" + k + "'",
                "'" + activity_id_list[counter] + "'",
                "'" + k + "'",
                "'" + v["parent_activity_id"][0] + "'",
                "'" + v["parent_experiment_id"][0] + "'",
                "'" + v["required_model_components"][0] + "'",
                "'" + " ".join(v["additional_allowed_model_components"][:]) + "'",
                "'" + v["experiment"].replace("'", "") + "'",
                "'" + " ".join(v["sub_experiment_id"][:]) + "'",
            )
        )

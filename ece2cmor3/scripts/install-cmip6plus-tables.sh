...
if [ "$#" -eq 1 ]; then
 ...
 # Create a new directory which from structure resembles the CMIP6 tables repository with links to the two repositories which contain for CMIP6Plus the required files:
 ...
 # Then go to your ece2cmor3 directory, activate your conda env as usual, and run
 ...
 #ece2cmor --varlist varlist.json \
 #         --meta metadata-nemo.json \
 #         --nemo \
 #         --exp $EXPID \
 #         --tabledir ${dir_for_tables}/cmip6plus-tables \
 #         --tableprefix MIP \
 #         /path/to/$EXPID/output/nemo/$LEG

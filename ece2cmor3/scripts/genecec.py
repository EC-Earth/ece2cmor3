...
if len(sys.argv) == 2:
    ...
    if os.path.isfile(config_filename) == False:  # Checking if the config file exists
        print(
            error_message, " The config file ", config_filename, "  does not exist.\n"
        )
        sys.exit()
    ...
    output_dir_name = config[
        "output_dir_name"
    ]  # output_dir_name                = 'output-control-files/'                          # Default: output-control-files/
    activate_pextra_mode = config[
        "activate_pextra_mode"
    ]  # activate_pextra_mode           = False                                            # Default: False
    add_request_overview = config[
        "add_request_overview"
    ]  # add_request_overview           = False                                            # Default: True
    ece2cmor_root_directory = config[
        "ece2cmor_root_directory"
    ]  # ece2cmor_root_directory        = '~/cmorize/ece2cmor3/'                           # Default: ~/cmorize/ece2cmor3/
    ...
    if os.path.isdir(expanded_ece2cmor_root_directory) == False:
        print(
            error_message,
            " The ece2cmor root directory ",
            expanded_ece2cmor_root_directory,
            " does not exist.\n",
        )
        sys.exit()
    if os.path.isfile(expanded_ece2cmor_root_directory + "/environment.yml") == False:
        print(
            ...
            " The ece2cmor root directory ",
            expanded_ece2cmor_root_directory,
            " is not an ece2cmor root directory.\n",
        )
        sys.exit()
    ...
    command_show_version = (
        'git describe --tags | sed "s/^/ Using ece2cmor git revision: /"; echo;'
    )
    ...
    for mip in dq.coll["mip"].items:
        ...
        if mip_name == "CMIP" or mip_name == "ScenarioMIP":
            if mip_name == "CMIP":
                ...
                print(
                    "\n Aborting genecec: programmer error: no case for: ",
                    mip_name,
                    " in joined MIP treatment.\n",
                )
                sys.exit()

            for model_configuration in sorted(ece_configurations.keys()):
                mip_list = ece_configurations[model_configuration]
                mip_label = mip_list.replace(
                    ",", "."
                )  # Convert the comma separated list into a dot separated list because this is what comes out from genecec-per-mip-experiment.sh
                ...
                for u in dq.inx.iref_by_sect[mip.uid].a["experiment"]:
                    ...
                    filename_file_def_nemo_opa = (
                        subdirname_experiment + "/file_def_nemo-opa.xml"
                    )
                    ece_configuration_dir = (
                        cmip6_base_dir_name
                        + mip_name
                        + "/"
                        + model_configuration
                    ...
                    command_02 = (
                        "sed -i -e 's/uoce_e3u_vsum_e2u_cumul. freq_op=.1ts/uoce_e3u_vsum_e2u_cumul/' "
                        + filename_file_def_nemo_opa
                    )
                    command_03 = "sed -i -e '/deptho/d' " + filename_file_def_nemo_opa
                    command_c = (
                        'sed -i \'s/enabled="True" field_ref="transport/enabled="False" field_ref="transport/\' '
                        + subdirname_experiment
                        + "/file_def_nemo*"
                    )
                    command_04 = (
                    ...
                    command_05 = (
                        "drq2varlist --drq cmip6-data-request/cmip6-data-request-"
                        + mip_label
                        + "-"
                        + ex.label
                        + "-t"
                    ...
                    command_07 = (
                        "convert_component_to_flat_json "
                        + ece_configuration_dir
                        + "/cmip6-data-request-varlist-"
                        + mip_name
                        + "-"
                    ...
                    command_10 = (
                        "cat "
                        + ece_configuration_dir
                        + "/request-overview.available.txt "
                        + ece_configuration_dir
                        + "/volume-estimate.txt > "
                        + ece_configuration_dir
                        + "/request-overview-"
                    ...
                    command_11 = (
                        "rm -f " + ece_configuration_dir + "/volume-estimate.txt"
                    )
                    command_12 = (
                        "rm -f "
                        + ece_configuration_dir
                    ...
                    if mip_name in ec_earth_mips:
                        ...
                        if ex.tier[0] in experiment_tiers_included:
                            ...
                            os.system(
                                command_04
                            )  # Rename directory names for joint MIPs
                            os.system(
                                command_05
                            )  # Produce the ec-earth component json data request variant, the so called varlist.json
                            os.system(
                                command_06
                            ...
                            if add_request_overview:
                                os.system(
                                    command_07
                                )  # Convert the json data request file which contains the EC-Earth components to a flat json data request file for checkvars
                                os.system(
                                    command_08
                                )  # Execute checkvars --asciionly for the flat json data request file which includes the preferences
                                os.system(command_09)  # Remove the flat json file
                                os.system(
                                    command_10
                                )  # Concatenate volume estimates to the request overview file
                            os.system(command_11)  # Remove volume estimate file
                            os.system(
                                command_12
                            )  # Remove the intermediate request-overview.available.txt file
                            experiment_counter = experiment_counter + 1
                        else:
                            print(
                                " Tier {} experiments are not included: Skipping: {}".format(
                                    ex.tier[0], command_01
                                )
                            )
                    else:
                        print(
                            " EC-Earth3 does not participate in {:11}: Skipping: {}".format(
                                mip_name, command_01
                            )
                        )
            ...
            for u in dq.inx.iref_by_sect[mip.uid].a["experiment"]:
                ...
                if mip_name in ec_earth_mips:
                    ...
                    if (
                        ex.tier[0] in experiment_tiers_included
                        or (ex.tier[0] == 2 and ex.label == "amip-lfmip-pdLC")
                        or (ex.tier[0] == 2 and ex.label == "amip-lfmip-rmLC")
                        or (ex.tier[0] == 2 and ex.label == "hist-piAer")
                        or (ex.tier[0] == 2 and ex.label == "histSST-piAer")
                        or (ex.tier[0] == 2 and ex.label == "piClim-2xdust")
                        or (ex.tier[0] == 2 and ex.label == "piClim-2xss")
                        or (ex.tier[0] == 2 and ex.label == "piClim-BC")
                        or (ex.tier[0] == 3 and ex.label == "ssp370-lowNTCFCH4")
                        or (ex.tier[0] == 2 and ex.label == "ssp370pdSST")
                        or (ex.tier[0] == 2 and ex.label == "ssp370SST-lowAer")
                        or (ex.tier[0] == 2 and ex.label == "ssp370SST-lowBC")
                        or (ex.tier[0] == 3 and ex.label == "ssp370SST-lowNTCFCH4")
                        or (ex.tier[0] == 2 and ex.label == "land-cClim")
                        or (ex.tier[0] == 2 and ex.label == "land-cCO2")
                        or (ex.tier[0] == 2 and ex.label == "land-noShiftCultivate")
                        or (ex.tier[0] == 2 and ex.label == "piClim-histaer")
                        or (ex.tier[0] == 2 and ex.label == "piClim-histall")
                    ):
                        # os.system(command_x1)    # Just set the toce fields false again because we still face troubles with them
                        # os.system(command_x2)    # Delete the line with sfdsi_2 from the file_def_nemo-opa.xml files
                        os.system(command_01)
                        ...
                        for conf in model_configuration:
                            ...
                            command_09 = (
                                "rm -f " + "cmip6-data-request-varlist*-flat.json"
                            )
                            command_10 = (
                                "cat "
                                + subdirname_experiment
    ...
    if os.path.isdir(
        cmip6_base_dir_name + "CMIP/EC-EARTH-AOGCM/cmip6-experiment-CMIP-piControl/"
    ):
        command_a = (
            "cp -r "
            + cmip6_base_dir_name
            + "CMIP/EC-EARTH-AOGCM/cmip6-experiment-CMIP-piControl/ "
            + cmip6_base_dir_name
            + "test-all-ece-mip-variables/"
        )
    else:
        command_a = (
            "cp -r "
            + cmip6_base_dir_name
            + "CMIP/cmip6-experiment-CMIP-piControl/ "
            + cmip6_base_dir_name
            + "test-all-ece-mip-variables/"
        )
    command_b = (
        ...
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/file_def_nemo-*"
    )
    command_c = (
        'sed -i \'s/enabled="True" field_ref="transport/enabled="False" field_ref="transport/\' '
        + cmip6_base_dir_name
    ...
    command_d = (
        "echo 'This directory is intended for the maintainers only. In order to be able to test all NEMO OPA & LIM output by running one experiment, all those fields are enabled in the OPA & LIM file_def files in this directory. And in order to be able to test all IFS output by running one experiment, all available IFS fields are enabled in the ppt files.' > "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/README"
    )
    command_e = (
        "rm -f "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/ppt* "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/cmip6-data-request-varlist-CMIP-piControl-EC-EARTH-AOGCM.json "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/volume-estimate-CMIP-piControl-EC-EARTH-AOGCM.txt "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/request-overview*.txt"
    )
    command_f = "drq2ppt --allvars"
    command_g = (
        "mv -f ppt0000000000 pptdddddd* "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/; rm -f volume-estimate-ifs.txt"
    )
    command_h = (
        "drq2varlist --allvars --ececonf EC-EARTH-AOGCM   --varlist "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/ece-cmip6-data-request-varlist-all-EC-EARTH-AOGCM.json"
    )
    command_i = (
        "drq2varlist --allvars --ececonf EC-EARTH-CC      --varlist "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/ece-cmip6-data-request-varlist-all-EC-EARTH-CC.json"
    )
    command_j = (
        "drq2varlist --allvars --ececonf EC-EARTH-AerChem --varlist "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/ece-cmip6-data-request-varlist-all-EC-EARTH-AerChem.json"
    )
    command_k = (
        "rm -f "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/lpjg_cmip6_output.ins; ln -s ../../../lpjg_cmip6_output.ins lpjg_cmip6_output.ins; mv -f lpjg_cmip6_output.ins "
        + cmip6_base_dir_name
        + "test-all-ece-mip-variables/"
    )
    ...
    os.system(
        command_a
    )  # Create a new subdirectory for testing all available variables in the file_def files
    ...
    os.system(
        command_e
    )  # Remove the piControl ppt, json data request and volume estimate files from the test-all-ece-mip-variables directory
    os.system(
        command_f
    )  # Create the ppt files which include all IFS available variables
    os.system(
        command_g
    )  # Move the ppt files which include all IFS available variables to the test-all-ece-mip-variables directory and remove the volume estimate file.
    os.system(
        command_h
    )  # Create the json data request file which includes all available variables for EC-Earth3-AOGCM
    os.system(
        command_i
    )  # Create the json data request file which includes all available variables for EC-Earth3-CC
    os.system(
        command_j
    )  # Create the json data request file which includes all available variables for EC-Earth3-AerChem
    os.system(
        command_k
    )  # Remove the piControl LPJG instruction file, and add a link to the instruction file which includes all available LPJG variables.

    command_fix_s245_s370 = (
        "./apply-the-s126-s585-request-for-s245-370.sh " + cmip6_base_dir_name
    )
    os.system(
        command_fix_s245_s370
    )  # See issue 517: ScenarioMIP requests for s245 & s370 are taken equal to the ones of s585 & s126.
    ...
    if not activate_pextra_mode:
        ...
        command_rcm_plev_mv = (
            "mv -f rcm-dynamic-plev23-forcing rcm-dynamic-plev36-forcing "
            + output_dir_name
        )

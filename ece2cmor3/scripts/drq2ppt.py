...
def get_sample_freq(task):
    name, axis, levs = cmor_target.get_z_axis(task.target)
...
def write_ppt_files(tasks):
    ...
    for freq in freqgroups:
        ...
        for task in freqgroups[freq]:
            name, zaxis, levs = cmor_target.get_z_axis(task.target)
        ...
        mfp2df = sorted(list(map(lambda x: x.var_id if x.tab_id == 128 else x.__hash__(), set(mfp2df))))
        mfpphy = sorted(list(map(lambda x: x.var_id if x.tab_id == 128 else x.__hash__(), set(mfpphy))))
        mfp3dfs = sorted(list(map(lambda x: x.var_id if x.tab_id == 128 else x.__hash__(), set(mfp3dfs))))
        mfp3dfp = sorted(list(map(lambda x: x.var_id if x.tab_id == 128 else x.__hash__(), set(mfp3dfp))))
        mfp3dfv = sorted(list(map(lambda x: x.var_id if x.tab_id == 128 else x.__hash__(), set(mfp3dfv))))

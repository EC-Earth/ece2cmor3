...
def main():
    parser = argparse.ArgumentParser(
        description="Generate the LPJ-GUESS instruction files and estimate the volume of "
    ...
    try:
        if getattr(args, "vars", None) is not None:
            ...
            taskloader.load_tasks_from_drq(
                args.drq, active_components=["lpjg"], check_prefs=False
            )
            ...
            taskloader.load_tasks_from_drq(
                os.path.join(
                    os.path.dirname(__file__), "..", "resources", "permanent-tasks.json"
                ),
    ...
    for task in ece2cmorlib.tasks:
        ...
        if task.target.frequency == "yr":
            instruction_file.write(
                'file_{}_yearly "{}_yearly.out"{}'.format(
                    task.target.variable, task.target.variable, "\n"
                )
            )
        elif task.target.frequency == "mon":
            instruction_file.write(
                'file_{}_monthly "{}_monthly.out"{}'.format(
                    task.target.variable, task.target.variable, "\n"
                )
            )
        elif task.target.frequency == "day":
            instruction_file.write(
                'file_{}_daily "{}_daily.out"{}'.format(
                    task.target.variable, task.target.variable, "\n"
                )
            )
        elif task.target.frequency == "3hr":
            print(
                " LPJ-GUESS does not provide three hourly (3hr) output: ",
                task.target.variable,
                task.target.table,
                task.target.frequency,
            )
            ...
            print(
                " LPJ-GUESS does not provide six hourly (6hr) output: ",
                task.target.variable,
                task.target.table,
                task.target.frequency,
            )
            ...
            print(
                " LPJ-GUESS does not provide three hourly instantaneous (3hrPt) output: ",
                task.target.variable,
                task.target.table,
                task.target.frequency,
            )
            ...
            print(
                " LPJ-GUESS does not provide subhourly instantaneous (subhrPt) output: ",
                task.target.variable,
                task.target.table,
                task.target.frequency,
            )
        ...
        if task.target.frequency == "yr":
            ...
            layer_number_due_to_freq = (
                0  # LPJ-GUESS does not provide three hourly (3hr) output
            )
        elif task.target.frequency == "6hr":
            layer_number_due_to_freq = (
                0  # LPJ-GUESS does not provide six hourly (6hr) output
            )
        elif task.target.frequency == "yrPt":
            # layer_number_due_to_freq = 0             # LPJ-GUESS does not provide yearly instantaneous (yrPt) output
            layer_number_due_to_freq = 1
        elif task.target.frequency == "3hrPt":
            layer_number_due_to_freq = 365.25 * 8.0
        elif task.target.frequency == "fx":
            layer_number_due_to_freq = 0
        elif task.target.frequency == "monC":
            layer_number_due_to_freq = (
                1.0 / 30.0
            )  # Number of climate points: 1 per 30 year?
        elif task.target.frequency == "subhrPt":
            ...
            layer_number_due_to_freq = (
                0  # Because there won't be any subhourly output from LPJ-GUESS.
            )
        else:
            print(
    ...
    print(
        "\n With a 2D layer equivalent of ",
        total_layer_equivalent,
        " the LPJ-GUESS Volume estimate for this CMIP6 data request at T255 grid is ",
        total_layer_equivalent * 0.12 / 1000.0,
        " GB per year\n",
    )
    print(
        " The number of variables which is available from LPJ-GUESS in EC-Erth3 for this experiment is",
        count,
    )
    ...
    volume_estimate = open("volume-estimate-lpj-guess.txt", "w")
    volume_estimate.write(
        "Heuristic volume estimate for the raw EC-Earth3 LPJG output on the T255        grid: {:6} GB per year{}".format(
            round((total_layer_equivalent * 0.12 / 1000.0) / hf, 1), "\n"
        )
    )
    volume_estimate.close()

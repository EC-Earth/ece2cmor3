...
def write_varlist_ascii(targets, opath, print_all_columns):
    ...
    if print_all_columns:
        ...
        ofile.write(
            "{:10} {:20} {:5} {:45} {:115} {:20} {:85} {:140} {:20} {} {}".format(
                "table",
                "variable",
                "prio",
                "dimensions",
                ...
                "unit",
                "link",
                "list of MIPs which request this variable",
                "comment_author",
                "comment",
                "\n",
            )
...
def main():
    ...
    loaded_targets = sorted(
        list(set(loaded)), key=lambda tgt: (tgt.table, tgt.variable)
    )
    ignored_targets = sorted(
        list(set(ignored)), key=lambda tgt: (tgt.table, tgt.variable)
    )
    identified_missing_targets = sorted(
        identified_missing, key=lambda tgt: (tgt.table, tgt.variable)
    )
    ...
    if args.output:
        ...
        if args.verbose:
            if not args.asciionly:
                write_varlist_excel(
                    loaded_targets, args.output + ".available.xlsx", args.withping
                )
                write_varlist_excel(
                    ignored_targets, args.output + ".ignored.xlsx", args.withping
                )
                write_varlist_excel(
                ...
                write_varlist_excel(
                    missing_targets, args.output + ".missing.xlsx", args.withping
                )

            if args.drq[-4:] == "xlsx":
                write_varlist_ascii(
                    loaded_targets, args.output + ".available.txt", True
                )
                ...
                write_varlist_ascii(
                    loaded_targets, args.output + ".available.txt", False
                )

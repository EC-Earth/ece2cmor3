 ...
 if [ "$#" -eq 2 ]; then
  ...
  #export ECE2CMOR3_IFS_CLEANUP=FALSE
   ...
   if [ ${arg2next} -lt 252 ] ; then
    ...
    echo ' ' sbatch --job-name=cmorise-s585-${arg1}-${arg2} ${arg0} ${arg1} ${arg2}
    sbatch --job-name=cmorise-s585-${arg1}-${arg2} ${arg0} ${arg1} ${arg2}
   else
  ...
  echo '  Or use:'
  echo '   for j in {166..181}; do sbatch --job-name=cmorise-s585-ifs-$j ' $0 ' ifs $j; done'
  echo '   for j in {166..181}; do sbatch --job-name=cmorise-s585-nemo-$j ' $0 ' nemo $j; done'
  echo
 fi

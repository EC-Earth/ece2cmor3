...
# LEG       is the leg number for the current job to cmorise. Note for instance leg number one is written as 001.
 ...
 if [ "$#" -eq 5 ]; then
  ...
  #TABLEPREFIX=CMIP6
   ...
   #/perm/ecme6608/FORCEs_output/2014/001
   ECEDIR=/perm/ecme6608/FORCEs_output/2014/$LEG
   METADATA=${PWD}/../../resources/metadata-templates/forces-wrf/metadata-cmip6-CMIP-historical-EC-EARTH-AerChem-ifs-template-forces-wrf.json
   TEMPDIR=${SCRATCH}/temp-cmor-dir/$EXPID/$COMPONENT/$LEG
   VARLIST=${PWD}/../../resources/miscellaneous-data-requests/forces-wrf/cmip6-data-request-varlist-CMIP-historical-EC-EARTH-AerChem-forces-wrf.json
   ODIR=${SCRATCH}/cmorised-results/forces-wrf/$EXPID/$VERSION
  ...
  #mv -f $EXPID-$COMPONENT-$LEG-*.log $ODIR/logs/
   mv -f $EXPID-2014-$LEG-*.log $ODIR/logs/
   if [ -d $TEMPDIR ]; then rm -rf $TEMPDIR; fi
  ...
  echo " Or use:"
  echo "  for i in {065..065}; do sbatch --qos=np --cpus-per-task=96 --job-name=cmorise-ifs-\$i  $0 96 ifs  \$i fhi1 v001; done"
  echo "  for i in {065..065}; do sbatch --qos=nf --cpus-per-task=1  --job-name=cmorise-nemo-\$i $0  1 nemo \$i fhi1 v001; done"
  echo "  for i in {065..065}; do sbatch --qos=nf --cpus-per-task=1  --job-name=cmorise-lpjg-\$i $0  1 lpjg \$i fhi1 v001; done"
  echo "  for i in {065..065}; do sbatch --qos=nf --cpus-per-task=1  --job-name=cmorise-tm5-\$i  $0  1 tm5  \$i fhi1 v001; done"
  echo
 fi

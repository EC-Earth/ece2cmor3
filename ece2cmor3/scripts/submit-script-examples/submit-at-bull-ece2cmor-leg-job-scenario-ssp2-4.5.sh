...
# LEG       is the leg number for the current job to cmorise. Note for instance leg number one is written as 001.
 ...
 if [ "$#" -eq 2 ]; then
   ...
   METADATA=/nfs/home/users/reerink/ec-earth-3/trunk/runtime/classic/ctrl/cmip6-output-control-files/ScenarioMIP/EC-EARTH-AOGCM/cmip6-experiment-ScenarioMIP-ssp245/metadata-cmip6-ScenarioMIP-ssp245-EC-EARTH-AOGCM-$COMPONENT-template.json
   TEMPDIR=/lustre3/projects/CMIP6/reerink/temp-cmor-dir/$EXP/$COMPONENT/$LEG
   VARLIST=/nfs/home/users/reerink/ec-earth-3/trunk/runtime/classic/ctrl/cmip6-output-control-files/ScenarioMIP/EC-EARTH-AOGCM/cmip6-experiment-ScenarioMIP-ssp245/cmip6-data-request-varlist-ScenarioMIP-ssp245-EC-EARTH-AOGCM.json
   ODIR=/lustre3/projects/CMIP6/reerink/cmorised-results/cmor-cmip-scenario-ssp2-4.5/$EXP
   ...
   if [ ${arg2next} -lt 252 ] ; then
    ...
    echo ' ' sbatch --job-name=cmorise-s245-${arg1}-${arg2} ${arg0} ${arg1} ${arg2}
    sbatch --job-name=cmorise-s245-${arg1}-${arg2} ${arg0} ${arg1} ${arg2}
   else
  ...
  echo '  Or use:'
  echo '   for j in {166..181}; do sbatch --job-name=cmorise-s245-ifs-$j ' $0 ' ifs $j; done'
  echo '   for j in {166..181}; do sbatch --job-name=cmorise-s245-nemo-$j ' $0 ' nemo $j; done'
  echo
 fi

...
# LEG       is the leg number for the current job to cmorise. Note for instance leg number one is written as 001.
 ...
 if [ "$#" -eq 4 ]; then
   ...
   VARLIST=${PWD}/../../resources/miscellaneous-data-requests/time-invariant-request/varlist-APfx-LPfx-OPfx.json
  ...
  echo " For instance:"
  echo "  sbatch --cpus-per-task=5 --job-name=cmorise-ifs-LPfx  $0 ifs  001 t001 v010"
  echo "  sbatch --cpus-per-task=1 --job-name=cmorise-nemo-OPfx $0 nemo 001 t001 v010"
  echo
 fi

...
# LEG       is the leg number for the current job to cmorise. Note for instance leg number one is written as 001.
 ...
 if [ "$#" -eq 2 ]; then
   ...
   EXP=t264
   ECEDIR=/lustre2/projects/model_testing/reerink/ec-earth-3/trunk/$EXP/output/$COMPONENT/$LEG
   ...
   ODIR=/lustre2/projects/model_testing/reerink/cmorised-results/cmor-cmip-piControl/$EXP
  ...
  #export ECE2CMOR3_IFS_CLEANUP=FALSE
   ...
   if [ ${arg2next} -lt 601 ] ; then
    ...
    echo ' ' sbatch --job-name=cmorise-piControl-${arg1}-${arg2} ${arg0} ${arg1} ${arg2}
    sbatch --job-name=cmorise-piControl-${arg1}-${arg2} ${arg0} ${arg1} ${arg2}
   else
  ...
  echo '  Or use:'
  echo '   for j in {100..115}; do sbatch --job-name=cmorise-piControl-ifs-$j ' $0 ' ifs $j; done'
  echo '   for j in {100..115}; do sbatch --job-name=cmorise-piControl-nemo-$j ' $0 ' nemo $j; done'
  echo
 fi

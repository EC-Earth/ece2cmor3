...
# LEG       is the leg number for the current job to cmorise. Note for instance leg number one is written as 001.
 ...
 if [ "$#" -eq 2 ]; then
   ...
   if [ ${arg2next} -lt 252 ] ; then
    ...
    echo ' ' sbatch --job-name=cmorise-s126-${arg1}-${arg2} ${arg0} ${arg1} ${arg2}
    sbatch --job-name=cmorise-s126-${arg1}-${arg2} ${arg0} ${arg1} ${arg2}
   else
  ...
  echo '  Or use:'
  echo '   for j in {166..181}; do sbatch --job-name=cmorise-s126-ifs-$j ' $0 ' ifs $j; done'
  echo '   for j in {166..181}; do sbatch --job-name=cmorise-s126-nemo-$j ' $0 ' nemo $j; done'
  echo
 fi

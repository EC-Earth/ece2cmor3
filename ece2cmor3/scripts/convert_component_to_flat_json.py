...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        input_json_file = sys.argv[
            1
        ]  # Reading the data request file name from the argument line
        if (
            os.path.isfile(input_json_file) == False
        ):  # Checking if the data request file exists
            print(
                error_message,
                " The data request file ",
                input_json_file,
                " does not exist.\n",
            )
            sys.exit()
        ...
        output_json_file = os.path.basename(input_json_file).replace(
            ".json", "-flat.json"
        )
        ...
        if "ifs" in data_request:
            ...
            print(
                ...
                "The file",
                sys.argv[1],
                "is a flat json already, thefore it is not converted but copied instead.",
            )
        ...
        print(
            "  ",
            sys.argv[0],
            "control-output-files/output-control-files-v196/cmip6/CMIP/EC-EARTH-AOGCM/cmip6-experiment-CMIP-historical/cmip6-data-request-varlist-CMIP-historical-EC-EARTH-AOGCM.json",
        )
        print(
            "  ",
            sys.argv[0],
            "../resources/miscellaneous-data-requests/lamaclima/lamaclima-data-request-varlist-EC-EARTH-Veg.json",
        )
        print(
            "  ",
            sys.argv[0],
            "control-output-files/output-control-files-v196/cmip6/AerChemMIP/cmip6-experiment-AerChemMIP-hist-1950HC/cmip6-data-request-varlist-AerChemMIP-hist-1950HC-EC-EARTH-AerChem.json",
        )
        print()

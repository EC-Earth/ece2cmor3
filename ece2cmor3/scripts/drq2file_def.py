...
basic_file_def_file_name = (
    expanduser("~")
    + "/cmorize/ece2cmor3/ece2cmor3/resources/xios-nemo-file_def-files/basic-cmip6-file_def_nemo.xml"
)
...
def main():
    parser = argparse.ArgumentParser(
        description="Create NEMO XIOS file_def input files for a given CMIP6 data request"
    )
    ...
    try:
        if getattr(args, "vars", None) is not None:
            ...
            taskloader.load_tasks_from_drq(
                args.drq, active_components=["nemo"], check_prefs=False
            )
    ...
    for field in root_basic_file_def.findall(".//field[@id]"):
        for task in ece2cmorlib.tasks:
            if (
                field.attrib["name"] == task.target.variable
                and field.attrib["table"] == task.target.table
            ):
    ...
    print(
        ...
        " the NEMO Volume estimate for this CMIP6 data request is ",
        total_layer_equivalent * 0.43 / 1000.0,
        " GB per year\n",
    )
    print(
        " The number of variables which is enabled in", file_def_file_name, " is", count
    )
    ...
    volume_estimate = open("volume-estimate-nemo.txt", "w")
    volume_estimate.write(
        "Heuristic volume estimate for the raw EC-Earth3 NEMO output on the ORCA1L75    grid: {:6} GB per year{}".format(
            round((total_layer_equivalent * 0.43 / 1000.0) / hf, 1), "\n"
        )
    )
    volume_estimate.write(
        "Heuristic volume estimate for the raw EC-Earth3 NEMO output on the ORCA025L75  grid: {:6} GB per year{}".format(
            round((total_layer_equivalent * 5.76 / 1000.0) / hf, 1), "\n"
        )
    )
    volume_estimate.close()
    ...
    for file_element in root_opa.findall("./file_group/file"):
        ...
        model_component = re.search(
            "_(.+?)_", file_element.attrib["name_suffix"]
        ).group(1)
    ...
    if args.compact:
        ...
        root_opa_enabled_only = (
            tree_opa_enabled_only.getroot()
        )  # This root has two indices: the 1st index refers to field_definition-element, the 2nd index refers to the field-elements
        ...
        root_lim_enabled_only = (
            tree_lim_enabled_only.getroot()
        )  # This root has two indices: the 1st index refers to field_definition-element, the 2nd index refers to the field-elements
        ...
        root_pisces_enabled_only = (
            tree_pisces_enabled_only.getroot()
        )  # This root has two indices: the 1st index refers to field_definition-element, the 2nd index refers to the field-elements

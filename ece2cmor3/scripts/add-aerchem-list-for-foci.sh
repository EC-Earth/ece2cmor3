 ...
 if [ add_aerchem_foci_variables ]; then
  ...
  for i in $(seq 29); do
   ...
   if [ "${i}" -eq 13 ] || [ "${i}" -eq 14 ]; then
    ...
    echo '            "comment": "'Mode' refers to the mode of the M7 aerosol scheme (Vignati et al., 2004). Mass fraction in the atmosphere (dry mass). 'Aerosol' means the system of suspended liquid or solid particles in air (except cloud droplets) and their carrier gas, the air itself. 'Ambient_aerosol' means that the aerosol is measured or modelled at the ambient state of pressure, temperature and relative humidity that exists in its immediate environment. 'Ambient aerosol particles' are aerosol particles that have taken up ambient water through hygroscopic growth. The extent of hygroscopic growth depends on the relative humidity and the composition of the particles.", ' | sed 's/\s*$//g' >> ${table_file_AER6hrPt}
   fi
   echo '            "dimensions": "longitude latitude alevel time",                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     ' | sed 's/\s*$//g' >> ${table_file_AER6hrPt} 

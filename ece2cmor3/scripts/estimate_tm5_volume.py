...
def main():
    parser = argparse.ArgumentParser(
        description="Estimates the volume of the output from TM5 for a given CMIP6 data "
    ...
    parser.add_argument(
        "--short", action="store_true", default=False, help="Leave out the tasklist"
    )
    ...
    try:
        if getattr(args, "vars", None) is not None:
            ...
            taskloader.load_tasks_from_drq(
                args.drq, active_components=["tm5"], check_prefs=False
            )
    ...
    for task in ece2cmorlib.tasks:
        ...
        if task.target.frequency == "yr":
            layer_number_due_to_freq = 1.0
        elif task.target.frequency == "mon":
            layer_number_due_to_freq = 12.0
        elif task.target.frequency == "day":
            layer_number_due_to_freq = 365.0
        elif task.target.frequency == "3hrPt":
            layer_number_due_to_freq = 0.0
        elif task.target.frequency == "6hrPt":
            layer_number_due_to_freq = 365.25 * 4.0
        elif task.target.frequency == "1hr":
            layer_number_due_to_freq = 365.25 * 24.0
            ...
            layer_number_due_to_freq = (
                0.0  # Because there won't be any subhourly output from TM5.
            )
        else:
            print(
        ...
        if len(zdim) == 0:
            ...
            if zdim[0] == "alevel":
                vertical_dim = 35.0  # 34 + 1 for ps
            elif zdim[0] == "alevhalf":
                vertical_dim = 36.0  # 34 + 1 for ps
            elif zdim[0] == "plev39":
                vertical_dim = 39.0
            elif zdim[0] == "plev19":
                vertical_dim = 19.0
        ...
        if task.target.table == "AERmonZ":
            per_freq[task.target.table] = (
                per_freq[task.target.table] + layers_per_var_per_yr / 120.0
            )
            # TM5 Volume estimate: and for all variables together:
            total_layer_equivalent = (
                total_layer_equivalent + layers_per_var_per_yr / 120.0
            )
        else:
            per_freq[task.target.table] = (
                per_freq[task.target.table] + layers_per_var_per_yr
            )
            # TM5 Volume estimate: and for all variables together:
    ...
    print(
        ...
        total_layer_equivalent * 0.04 / 1000.0,
        " GB per year\n",
    )
    print(
    ...
    volume_estimate = open("volume-estimate-tm5.txt", "w")
    volume_estimate.write(
        "Heuristic volume estimate for the raw EC-Earth3 TM5  output on the TM5 3x2 deg grid: {:6} GB per year{}".format(
            round((total_layer_equivalent * 0.04 / 1000.0) / hf, 1), "\n"
        )
    )
    volume_estimate.close()

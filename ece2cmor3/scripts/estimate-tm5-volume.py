...
def main():
    ...
    per_freq['CFsubhr']=0
    task_per_freq={}
    task_per_freq['AERmon']=0
    task_per_freq['AERday']=0
    task_per_freq['6hrLev']=0
    task_per_freq['3hr']=0
    task_per_freq['AERhr']=0
    task_per_freq['Amon']=0
    task_per_freq['CFday']=0
    task_per_freq['AERmonZ']=0
    task_per_freq['CFsubhr']=0
    for task in ece2cmorlib.tasks:
      count = count + 1
      if not getattr(args,'short',False):
        print ' {:15} {:9} {:15} {}'.format(task.target.variable, task.target.table, task.target.units, task.target.frequency)
      ...
      if task.target.table =='AERmonZ':
        per_freq[task.target.table]=per_freq[task.target.table]+layers_per_var_per_yr/120
        # TM5 Volume estimate: and for all variables together:
        total_layer_equivalent = total_layer_equivalent + layers_per_var_per_yr/120
      else:
    ...
    for i in per_freq:
      ...
      #else:
      print 'Table: {} \t tasks {} \tsize: {} GB/yr'.format(i,task_per_freq[i], per_freq[i]*0.04/1024)

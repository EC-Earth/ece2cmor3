...
def write_ifspar_xml_content(output_file, var):
    ...
    output_file.write('  <variable  cmip6_variable={:16}' \
                                  ' grib_code={:10}'      \
                                  ' convert={:16}'        \
                                  ' masked={:10}'         \
                                  ' interpolate={:10}'    \
...
def main():
   ...
   # Read the CMIP6 - CMIP7 mapping XML file (which is produced by running: ./cmip6-cmip7-variable-mapping.py v1.2.2.5 -r )

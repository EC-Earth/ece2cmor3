...
def main():
 ...
 #xpath_expression = xpath_path + "[@" + selected_attribute + "]"
  ...
  def inherit_message_2(attribute, ancestor_grade, chain_element, xpath_expression_in_chain, label):
      attribute_value = chain_element.get(attribute)
      if attribute_value:
       print(' {:11} {:20}: ancestor grade: {} {:30} {:60} {}'.format(attribute, attribute_value, ancestor_grade, chain_element.tag, xpath_expression_in_chain, label))
      else:
       print(' {:11} {:20}: ancestor grade: {} {:30} {:60} {}'.format(attribute, ''             , ancestor_grade, chain_element.tag, xpath_expression_in_chain, label))

  def inherit_attribute(attribute, starting_element, xpath_expression_in_chain, ancestor_grade, verbose):
       count = 0
        ...
        for element_in_chain_of_references in root_main.findall(xpath_expression_in_field_ref_chain):
         ...
         if attribute_from_chain_element:
          ...
          if preference_to_ancestors_of_chain_element:
           ...
           inherit_successful = inherit_attribute(attribute, element, xpath_expression_final_field_ref_in_chain, ancestor_grade, verbose)
  ...
  def inherit_attribute_from_ancestors(attribute, starting_element, xpath_expression_in_ancestor_chain, ancestor_grade, verbose):
       ...
       for ancestor_element in root_main.findall(xpath_expression_in_ancestor_chain):
        ...
        #label = 'inherits from ancestor grade {}'.format(ancestor_grade)
         ...
         ancestor_grade += 1
         inherit_attribute_from_ancestors(attribute, starting_element, xpath_expression_in_ancestor_chain, ancestor_grade, verbose)


  verbose_on_inheritance = False
   ...
   if element.get('field_ref'):
    ...
    for attribute in ['grid_ref', 'operation', 'unit', 'freq_op', 'freq_offset']:  # 'domain_ref'
      if element.get(attribute):
       ...
       attribute_inherited = inherit_attribute_via_field_ref_chain(attribute, element, chain_of_reference, verbose_on_inheritance)
       ...
       if preference_to_ancestors_of_chain_element == False:
        ...
        #print(' Try to inherit from any ancestors of the field_ref element ({}) at the end of the chain.'.format(chain_of_reference[-1]))
         ...
         inherit_attribute(attribute, element, xpath_expression_final_field_ref_in_chain, ancestor_grade, verbose_on_inheritance)
    ...
    for attribute in ['grid_ref', 'operation', 'unit', 'freq_op', 'freq_offset']:
      if element.get(attribute):
       ...
       inherit_attribute_from_ancestors(attribute, element, xpath_expression_in_ancestor_chain, ancestor_grade, verbose_on_inheritance)
 ...
 if True:
  ...
  write_field_group_to_xml_file(oifs_cmip7_xml_file, 'oifs_cmip7_lon_lat'                  , 'reduced_sfc'   , list_with_xml_lines_for_lon_lat               )
  write_field_group_to_xml_file(oifs_cmip7_xml_file, 'oifs_cmip7_lon_lat_time_tavg'        , 'reduced_sfc'   , list_with_xml_lines_for_lon_lat_time_tavg     )
  write_field_group_to_xml_file(oifs_cmip7_xml_file, 'oifs_cmip7_lon_lat_plev19_time_tavg' , 'reduced_plev19', list_with_xml_lines_for_lon_lat_plev19_time   )
  write_field_group_to_xml_file(oifs_cmip7_xml_file, 'oifs_cmip7_lon_lat_alevel_time_tavg' , 'reduced_ml'    , list_with_xml_lines_for_lon_lat_alevel_time   )
  write_field_group_to_xml_file(oifs_cmip7_xml_file, 'oifs_cmip7_lon_lat_plev3_time1'      , 'reduced_plev3' , list_with_xml_lines_for_lon_lat_plev3_time1   )
  ...
  write_field_group_to_xml_file(oifs_cmip7_xml_file, 'oifs_cmip7_lon_lat_time_height10m'   , 'reduced_sfc'   , list_with_xml_lines_for_lon_lat_time_height10m)
  # grid_ref probably incorrect for several of this mixed group:
  write_field_group_to_xml_file(oifs_cmip7_xml_file, 'oifs_cmip7_other'                    , 'reduced_sfc'   , list_with_xml_lines_for_other                 )

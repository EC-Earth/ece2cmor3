...
# create-nemo-only-list/nemo-only-list-cmip6-requested-variables.xlsx has been created by
...
ping_file_name_ocean = (
    expanduser("~") + "/ec-earth-3/trunk/runtime/classic/ctrl/ping_ocean_DR1.00.27.xml"
)
ping_file_name_seaIce = (
    expanduser("~") + "/ec-earth-3/trunk/runtime/classic/ctrl/ping_seaIce_DR1.00.27.xml"
)
ping_file_name_ocnBgchem = (
    expanduser("~")
    + "/ec-earth-3/trunk/runtime/classic/ctrl/ping_ocnBgChem_DR1.00.27.xml"
)
...
nemo_only_dr_nodummy_file_xlsx = (
    expanduser("~")
    + "/cmorize/ece2cmor3/ece2cmor3/scripts/create-nemo-only-list/nemo-only-list-cmip6-requested-variables.xlsx"
)
...
basic_flat_file_def_file_name = (
    expanduser("~")
    + "/cmorize/ece2cmor3/ece2cmor3/resources/xios-nemo-file_def-files/basic-flat-cmip6-file_def_nemo.xml"
)
...
if os.path.isfile(ping_file_name_ocean) == False:
    print((" The file ", ping_file_name_ocean, "  does not exist."))
    sys.exit(" stop")
if os.path.isfile(ping_file_name_seaIce) == False:
    print((" The file ", ping_file_name_seaIce, "  does not exist."))
    sys.exit(" stop")
if os.path.isfile(ping_file_name_ocnBgchem) == False:
    print((" The file ", ping_file_name_ocnBgchem, "  does not exist."))
    sys.exit(" stop")
...
for child in field_elements_Ocean:
    if exclude_dummy_fields and child.attrib["field_ref"].startswith("dummy_"):
        ...
        if "expr" in child.attrib:
            pinglistOcean_expr.append(child.attrib["expr"])
        else:
            pinglistOcean_expr.append("None")
...
for child in field_elements_SeaIce:
    if exclude_dummy_fields and child.attrib["field_ref"].startswith("dummy_"):
        ...
        if "expr" in child.attrib:
            pinglistSeaIce_expr.append(child.attrib["expr"])
        else:
            pinglistSeaIce_expr.append("None")
...
for child in field_elements_OcnBgchem:
    if exclude_dummy_fields and child.attrib["field_ref"].startswith("dummy_"):
        ...
        if "expr" in child.attrib:
            pinglistOcnBgchem_expr.append(child.attrib["expr"])
        else:
            pinglistOcnBgchem_expr.append("None")

total_pinglist_id = pinglistOcean_id + pinglistSeaIce_id + pinglistOcnBgchem_id
total_pinglist_field_ref = (
    pinglistOcean_field_ref + pinglistSeaIce_field_ref + pinglistOcnBgchem_field_ref
)
...
if exclude_dummy_fields:
    print(
        (
            "\n There are ",
            len(total_pinglist_id),
            "non-dummy variables taken from the shaconemo ping files.\n",
        )
    )
else:
    print(
        (
            "\n There are ",
            len(total_pinglist_id),
            "variables taken from the shaconemo ping files.\n",
        )
    )
...
for i in range(len(total_pinglist_id)):
    if total_pinglist_expr[i] != "None":
        if total_pinglist_expr[i].replace(" ", "").replace(
            "@", ""
        ) != total_pinglist_text[i].replace(" ", ""):
            print(
                (
                    " Mismatch between ping content and ping expr for variable",
                    total_pinglist_id[i],
                    ":",
                    total_pinglist_expr[i].replace(" ", "").replace("@", "")
                    + " and "
                    + total_pinglist_text[i].replace(" ", "")
                    + "\n",
                )
...
def create_element_lists(file_name, attribute_1, attribute_2):
    ...
    field_elements_attribute_1 = (
        []
    )  # The basic list in this routine containing the id attribute values
    field_elements_attribute_2 = (
        []
    )  # A list corresponding with the id list containing the grid_def attribute values
    fields_without_id_name = (
        []
    )  # This seperate list is created for fields which don't have an id (most of them have a name attribute, but some only have a field_ref attribute)
    fields_without_id_field_ref = (
        []
    )  # A corresponding list with the field_ref attribute values is created. The other list contains the name attribute values if available, otherwise the name is assumed to be identical to the field_ref value.
    attribute_overview = []

    text_elements = (
        []
    )  # A list corresponding with the id list containing the text                  values (i.e. the arithmic expressions as defined in the field_def file)
    unit_elements = (
        []
    )  # A list corresponding with the id list containing the unit        attribute values
    freq_offset_elements = (
        []
    )  # A list corresponding with the id list containing the freq_offset attribute values
    ...
    for group in range(0, len(roottree)):
        ...
        if roottree[group].tag != "field_group":
            if "grid_ref" in roottree[group].attrib:
                ...
                if "field_ref" in roottree[group].attrib:
                    ...
                    for field in roottree.findall(
                        './/field[@id="' + detected_field_ref + '"]'
                    ):
                        detected_grid_ref = field.attrib["grid_ref"]
                        if "unit" in field.attrib:
                            detected_unit = field.attrib["unit"]
                        else:
                            detected_unit = "no unit definition"
                        if "freq_offset" in field.attrib:
                            detected_freq_offset = field.attrib["freq_offset"]
                        else:
                        ...
                        field_elements_attribute_1.append(
                            roottree[group].attrib[attribute_1]
                        )
                        field_elements_attribute_2.append(
                            'grid_ref="' + detected_grid_ref + '"'
                        )
                        text_elements.append(roottree[group].text)
                        if "unit" in roottree[group].attrib:
                            unit_elements.append(roottree[group].attrib["unit"])
                        ...
                        if "freq_offset" in roottree[group].attrib:
                            freq_offset_elements.append(
                                roottree[group].attrib["freq_offset"]
                            )
                        else:
                    ...
                    print(
                        (
                            " ERROR: No field_ref and no grid_ref attribute for this variable ",
                            roottree[group].attrib[attribute_1],
                            " which has no field_group element level. This element has the attributes: ",
                            roottree[group].attrib,
                        )
                    )
        ...
        for child in elements:
            if child.tag != "field":
                print(
                    (
                        ' At expected "field" element level, a deviating tag ',
                        child.tag,
                        " is detected.",
                        list(child.attrib.keys()),
                    )
                )
            attribute_overview = attribute_overview + list(
                child.attrib.keys()
            )  # Merge each step the next list of attribute keys with the overview list
            ...
            if attribute_1 in child.attrib:
                ...
                if "unit" in child.attrib:
                    unit_elements.append(child.attrib["unit"])
                else:
                    unit_elements.append("no unit definition")
                if "freq_offset" in child.attrib:
                    freq_offset_elements.append(child.attrib["freq_offset"])
                    # print(child.attrib["freq_offset"], child.attrib["id"], file_name)
                else:
                    freq_offset_elements.append("no freq_offset")

                if attribute_2 in child.attrib:
                    field_elements_attribute_2.append(
                        'grid_ref="' + child.attrib[attribute_2] + '"'
                    )
                    ...
                    # print(' WARNING: No ', attribute_2, ' attribute for this variable: ', child.attrib[attribute_1], ' This element has the attributes: ', child.attrib)
                        ...
                        if "do include domain ref" == "do include domain ref":
                            ...
                            if "domain_ref" in roottree[group].attrib:
                                field_elements_attribute_2.append(
                                    'domain_ref="'
                                    + roottree[group].attrib["domain_ref"]
                                    + '"'
                                )
                            else:
                                print(
                                    (
                                        " ERROR: No ",
                                        "domain_ref",
                                        " attribute either for this variable: ",
                                        child.attrib[attribute_1],
                ...
                if "field_ref" in child.attrib:
                    ...
                    # print(' This variable {:15} has no id but it has a field_ref = {}'.format(child.attrib["name"], child.attrib["field_ref"]))
                        ...
                        fields_without_id_name.append(
                            child.attrib["field_ref"]
                        )  # ASSUMPTION about XIOS logic: in case no id and no name attribute are defined inside an element, it is assumed that the value of the field_ref attribute is taken as the value of the name attribute
                        fields_without_id_field_ref.append(child.attrib["field_ref"])
                    ...
                    print(
                        (
                            " ERROR: No ",
                            attribute_1,
                            "and no field_ref attribute either for this variable. This element has the attributes: ",
                            child.attrib,
                        )
                    )
    ...
    if not len(field_elements_attribute_1) == len(field_elements_attribute_2):
        print(" ERROR: The id and grid_ref list are not of equal length\n")
    if not len(fields_without_id_name) == len(fields_without_id_field_ref):
        print(" ERROR: The name and field_ref list are not of equal length\n")
    return (
        field_elements_attribute_1,
        field_elements_attribute_2,
        fields_without_id_name,
...
(
    field_def_nemo_opa_id,
    field_def_nemo_opa_grid_ref,
    no_id_field_def_nemo_opa_name,
    no_id_field_def_nemo_opa_field_ref,
    attribute_overview_nemo_opa,
    texts_opa,
    units_opa,
    freq_offsets_opa,
) = create_element_lists(
    field_def_file_directory + "field_def_nemo-opa.xml", "id", "grid_ref"
)
(
    field_def_nemo_lim_id,
    field_def_nemo_lim_grid_ref,
    no_id_field_def_nemo_lim_name,
    no_id_field_def_nemo_lim_field_ref,
    attribute_overview_nemo_lim,
    texts_lim,
    units_lim,
    freq_offsets_lim,
) = create_element_lists(
    field_def_file_directory + "field_def_nemo-lim.xml", "id", "grid_ref"
)
(
    field_def_nemo_pisces_id,
    field_def_nemo_pisces_grid_ref,
    no_id_field_def_nemo_pisces_name,
    no_id_field_def_nemo_pisces_field_ref,
    attribute_overview_nemo_pisces,
    texts_pisces,
    units_pisces,
    freq_offsets_pisces,
) = create_element_lists(
    field_def_file_directory + "field_def_nemo-pisces.xml", "id", "grid_ref"
)
(
    field_def_nemo_inerttrc_id,
    field_def_nemo_inerttrc_grid_ref,
    no_id_field_def_nemo_inerttrc_name,
    no_id_field_def_nemo_inerttrc_field_ref,
    attribute_overview_nemo_inerttrc,
    texts_inerttrc,
    units_inerttrc,
    freq_offsets_inerttrc,
) = create_element_lists(
    field_def_file_directory + "field_def_nemo-inerttrc.xml", "id", "grid_ref"
)


total_field_def_nemo_id = (
    field_def_nemo_opa_id
    + field_def_nemo_lim_id
    + field_def_nemo_pisces_id
    + field_def_nemo_inerttrc_id
)
total_field_def_nemo_grid_ref = (
    field_def_nemo_opa_grid_ref
    + field_def_nemo_lim_grid_ref
    + field_def_nemo_pisces_grid_ref
    + field_def_nemo_inerttrc_grid_ref
...
total_no_id_field_def_nemo_name = (
    no_id_field_def_nemo_opa_name
    + no_id_field_def_nemo_lim_name
    + no_id_field_def_nemo_pisces_name
    + no_id_field_def_nemo_inerttrc_name
)
total_no_id_field_def_nemo_field_ref = (
    no_id_field_def_nemo_opa_field_ref
    + no_id_field_def_nemo_lim_field_ref
    + no_id_field_def_nemo_pisces_field_ref
    + no_id_field_def_nemo_inerttrc_field_ref
)
total_attribute_overview_nemo_opa = (
    attribute_overview_nemo_opa
    + attribute_overview_nemo_lim
    + attribute_overview_nemo_pisces
    + attribute_overview_nemo_inerttrc
...
total_freq_offsets = (
    freq_offsets_opa + freq_offsets_lim + freq_offsets_pisces + freq_offsets_inerttrc
)
...
print(
    (
        ...
        len(total_no_id_field_def_nemo_name),
        "\n",
    )
)
...
print(
    (
        " In total there are",
        len(total_field_def_nemo_id),
        "fields defined with an id in the field_def files,",
        len(total_field_def_nemo_id) - len(list(set(total_field_def_nemo_id))),
        "of these id's occur twice.\n",
    )
)
...
for text in total_texts:
    if text == None:
        total_texts[total_texts.index(text)] = "None"
...
def check_which_list_elements_are_identical(list_of_attribute_1, list_of_attribute_2):
    ...
    for child in list_of_attribute_1:
        ...
        for identical_child in range(0, len(indices_identical_ids)):
            ...
            grid_ref_list.append(
                list_of_attribute_2[indices_identical_ids[identical_child]]
            )
        ...
        if not check_all_list_elements_are_identical(id_list):
            print(
                (
                    " WARNING: Different ids in sublist [should never occur] at positions:",
        ...
        if not check_all_list_elements_are_identical(grid_ref_list):
            print(
                (
                    " WARNING: The variable {:22} has different grid definitions, at positions: {:20} with grid: {}".format(
                        id_list[0], indices_identical_ids, grid_ref_list
                    )
                )
            )
        if message_occurence_identical_id and len(indices_identical_ids) > 1:
            print(
                (
                    " The variable {:22} occurs more than once, at positions: {:20} with grid: {}".format(
                        id_list[0], indices_identical_ids, grid_ref_list
                    )
                )
            )
        if len(indices_identical_ids) > 1:
            list_of_duplicate_variables.append(id_list[0])
...
vars_with_duplicate_id_definition_total = check_which_list_elements_are_identical(
    total_field_def_nemo_id, total_field_def_nemo_grid_ref
)
...
def load_checkvars_excel(excel_file):
    ...
    longname_colname = (
        "variable long name"  # Variable long name according to the data request
    )
    ...
    description_colname = (
        "extensive variable description"  # Description according to the data request
    )
    miplist_colname = "list of MIPs which request this variable"  # List of MIPs which request this variable in the data request
    model_component_colname = (
        "model component in ping file"  # The source of this data are the ping files
    )
    ping_units_colname = (
        "units as in ping file"  # The source of this data are the ping files
    )
    ping_comment_colname = (
        "ping file comment"  # The source of this data are the ping files
    )
    ...
    for sheetname in book.sheet_names():
        ...
        tablenames = [
            ...
            for c in sheet.col_slice(
                colx=coldict[table_colname], start_rowx=nr_of_header_lines
            )
        ]
        varnames = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[var_colname], start_rowx=nr_of_header_lines
            )
        ]
        varpriority = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[prio_colname], start_rowx=nr_of_header_lines
            )
        ]
        vardimension = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[dimension_colname], start_rowx=nr_of_header_lines
            )
        ]
        varlongname = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[longname_colname], start_rowx=nr_of_header_lines
            )
        ]
        varunit = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[unit_colname], start_rowx=nr_of_header_lines
            )
        ]
        weblink = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[link_colname], start_rowx=nr_of_header_lines
            )
        ]
        comments = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[comment_colname], start_rowx=nr_of_header_lines
            )
        ]
        description = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[description_colname], start_rowx=nr_of_header_lines
            )
        ]
        miplist = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[miplist_colname], start_rowx=nr_of_header_lines
            )
        ]
        model_component = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[model_component_colname], start_rowx=nr_of_header_lines
            )
        ]
        ping_units = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[ping_units_colname], start_rowx=nr_of_header_lines
            )
        ]
        ping_comment = [
            c.value
            for c in sheet.col_slice(
                colx=coldict[ping_comment_colname], start_rowx=nr_of_header_lines
            )
        ]
    return (
        tablenames,
...
if os.path.isfile(nemo_only_dr_nodummy_file_xlsx) == False:
    ...
    print(
        " The correct file can be generated by: ./create-nemo-only-list/create-nemo-only-list.sh"
    )
    sys.exit(" stop")
...
(
    ...
    dr_ping_units,
    dr_ping_comment,
) = load_checkvars_excel(nemo_only_dr_nodummy_file_xlsx)
...
for element_counter in range(0, len(dr_ping_component)):
    if dr_ping_component[element_counter] == "ocean":
        dr_ping_component[element_counter] = "opa"
    if dr_ping_component[element_counter] == "seaIce":
        dr_ping_component[element_counter] = "lim"
    if dr_ping_component[element_counter] == "ocnBgchem":
        dr_ping_component[element_counter] = "pisces"
################################################################################
...
for table in range(0, len(table_list_of_dr)):
    if not table_list_of_dr[table] in set(
        [
            "",
            "SImon",
            "Omon",
        ...
        print(
            (
                "\n No rule defined for the encountered table: ",
                table_list_of_dr[table],
...
for table in range(0, len(dr_table)):
    if (
        dr_table[table] == "SImon"
        or dr_table[table] == "Omon"
        or dr_table[table] == "Emon"
        or dr_table[table] == "EmonZ"
    ):
        dr_output_frequency[
            table
        ] = 'output_freq="1mo"'  # mo stands in XIOS for monthly output
    if (
        dr_table[table] == "SIday"
        or dr_table[table] == "Oday"
        or dr_table[table] == "Eday"
    ):
        dr_output_frequency[
            table
        ] = 'output_freq="1d"'  # d  stands in XIOS for dayly   output
    if dr_table[table] == "Oyr":
        dr_output_frequency[
            table
        ] = 'output_freq="1y"'  # y  stands in XIOS for yearly  output
    if dr_table[table] == "Oclim":
        dr_output_frequency[
            table
        ] = 'output_freq="1mo"'  # Save "mo", then in post process average it over the climatology intervals (e.g. 30 year intervals). See: ece2cmor3/resources/tables/CMIP6_Oclim.json ece2cmor3/resources/tables/CMIP6_CV.json
    if dr_table[table] == "Ofx":
        dr_output_frequency[
            table
        ] = 'output_freq="1y"'  # fx fixed: time invariant: operation=once thus time unit might not matter
    if dr_table[table] == "Odec":
        dr_output_frequency[
            table
        ] = 'output_freq="1y"'  # Save "y", then in post process average it over the decadal intervals
    if dr_table[table] == "3hr":
        dr_output_frequency[
            table
        ] = 'output_freq="3h"'  # h  stands in XIOS for hourly  output
################################################################################
...
if include_root_field_group_attributes:
    root_field_group_attributes = (
        'level="1" prec="4" default_value="1.e20" detect_missing_value="true"'
    )
else:
    root_field_group_attributes = ""
################################################################################
...
for i in range(0, len(dr_varname)):
    ...
    if not dr_varname[i] == "":
        ...
        if not dr_varname[i] == total_pinglist_id[index_in_ping_list]:
            print(
                (
                    " WARNING: Different names [should not occur]:",
                    dr_varname[i],
                    total_pinglist_id[index_in_ping_list],
                )
            )
        ...
        if include_grid_ref_from_field_def_files:
            ...
            if (
                not total_pinglist_field_ref[index_in_ping_list]
                in total_field_def_nemo_id
            ):
                nr_of_missing_fields_in_field_def = (
                    nr_of_missing_fields_in_field_def + 1
                )
                print(
                    (
                        " A field_ref in one of the ping files is not found in any of the field_def files: ",
                        nr_of_missing_fields_in_field_def,
                        total_pinglist_field_ref[index_in_ping_list],
                    )
                )
            else:
                nr_of_available_fields_in_field_def = (
                    nr_of_available_fields_in_field_def + 1
                )
                # print('available: ', nr_of_available_fields_in_field_def, total_pinglist_field_ref[index_in_ping_list])
                index_in_field_def_list = total_field_def_nemo_id.index(
                    total_pinglist_field_ref[index_in_ping_list]
                )
                ...
                texts = (
                    'fdf_expression="' + total_texts[index_in_field_def_list] + '"'
                )  # fdf expression: field_def file expression
                units = 'unit="' + total_units[index_in_field_def_list] + '"'
                freq_offsets = (
                    'freq_offset="' + total_freq_offsets[index_in_field_def_list] + '"'
                )
        else:
            # grid_ref = 'grid_ref="??"'
            grid_ref = ""
                ...
                if False:
                    if not hasattr(t, "time_operator"):
                        if True:
                            ...
                            print(
                                (
                                    " {:3}. The cmor variable {:16} {:6} area operator: {:14}   no time axis   {:18}  dimensions: {:34}  {}".format(
                                        view_counter,
                                        t.variable,
                                        t.table,
                                        getattr(t, "area_operator")[0],
                                        " ",
                                        getattr(t, "dimensions"),
                                        getattr(t, "cell_methods"),
                                    )
                                )
                            )
                    else:
                        if hasattr(t, "area_operator"):
                            if (
                                getattr(t, "time_operator")[0] in ["mean"]
                                and getattr(t, "area_operator")[0] in ["areacello"]
                                and getattr(t, "dimensions")
                                in [
                                ...
                                if False:
                                    ...
                                    print(
                                        (
                                            " {:3}. The cmor variable {:16} {:6} area operator: {:14}   time operator: {:18}  dimensions: {:34}  {}".format(
                                                view_counter,
                                                t.variable,
                                                t.table,
                                                getattr(t, "area_operator")[0],
                                                getattr(t, "time_operator")[0],
                                                getattr(t, "dimensions"),
                                                getattr(t, "cell_methods"),
                                            )
                                        )
                            ...
                            if getattr(t, "time_operator")[0] in ["mean"] and getattr(
                                t, "dimensions"
                            ) in [
                                "longitude latitude time",
                                "longitude latitude olevel time",
                                ...
                                if False:
                                    ...
                                    print(
                                        (
                                            " {:3}. The cmor variable {:16} {:6} no area operator {:12}   time operator: {:18}  dimensions: {:34}  {}".format(
                                                view_counter,
                                                t.variable,
                                                t.table,
                                                " ",
        ...
        for t in targets:
            if t.variable == dr_varname[i] and t.table == dr_table[i]:
                ...
                if False:
                    if cmor_table_units != str(dr_unit[i]):
                        print(
                            (
                                " The cmor units differ from cmor table and from the data request: ",
                                cmor_table_units,
                                " versus ",
                                str(dr_unit[i]),
                            )
                ...
                if not hasattr(t, "time_operator"):
                    ...
                    if getattr(t, "time_operator")[0] in [
                        "mean",
                        "mean where sea_ice",
                        "mean within years",
                    ]:
                        ...
                        if False:
                            cmor_table_freq_op = (
                                'freq_op="1ts"'  # The previous incorrect method
                            )
                        else:
                            if total_pinglist_text[index_in_ping_list] != None:
                                if "@" in total_pinglist_text[index_in_ping_list]:
                                    cmor_table_freq_op = (
                                        "freq_op=" + dr_output_frequency[i][12:]
                                    )  # Only in case the expression contains an @-operator: set freq_op equal to the output_freq
                                else:
        ...
        if total_pinglist_text[index_in_ping_list] != None:
            if "@" in total_pinglist_text[index_in_ping_list]:
                if cmor_table_operation != 'operation="average"':
                    print(
                        (
                            "\n WARNING: the time averaging operators @ are removed from the expression because a non time average variable is detected: {} becomes {} for {} {} with {}".format(
                                total_pinglist_text[index_in_ping_list],
                                total_pinglist_text[index_in_ping_list].replace(
                                    "@", ""
                                ),
                                dr_varname[i],
                                dr_table[i],
                    ...
                    total_pinglist_text[index_in_ping_list] = total_pinglist_text[
                        index_in_ping_list
                    ].replace("@", "")
        ...
        test_var_id_in_created_file_def = (
            "id_" + dr_output_frequency[i][13:15] + "_" + dr_varname[i]
        )
        if test_var_id_in_created_file_def in var_id_in_created_file_def:
            print(
                (
                    " \n WARNING: A duplicate id definition for "
                    + test_var_id_in_created_file_def
                    + " is made unique by adding an extension."
                )
            )
            ...
            index_var = var_id_in_created_file_def.index(
                test_var_id_in_created_file_def
            )
            if dr_table[index_var] in ["Omon", "Oclim"] and dr_table[i] in [
                "Omon",
                "Oclim",
            ]:
                print(
                    (
                        " SKIP: ",
                        dr_varname[index_var],
                        dr_table[i],
                        " because this variable - table combination is also asked for table",
                        dr_table[index_var],
                    )
                )
        ...
        if dr_table[i] in ["SImon", "SIday"] and dr_ping_component[i] != "lim":
            print(
                (
                    " \n WARNING: Table - model component matching issue with the variable:",
                    '"' + dr_varname[i] + '"',
                    "in table",
                    '"' + dr_table[i] + '"',
                    "orginating from model component",
                    '"' + dr_ping_component[i] + '"',
                    '. Model component wil be set to "lim".',
                )
            )
        ...
        if include_variable:
            ...
            flat_nemo_file_def_xml_file.write(
                "{:41} {:25} {:40} {:32} {:20} {:15} {:2} {:25} {:32} {:30} {:30} {:20} {:15} {:30} {:17} {:50} {:15} {:22} {:31} {:14} {:125} {:125} {:1030} {:1280} {:80} {:80} {:4} {:65} {:9}{}".format(
                    ...
                    ' description="'
                    + dr_description[i].replace("<", "less then ")
                    + '"',
                    ' ping_comment="'
                    + dr_ping_comment[i].replace('"', "'").replace("<", "less then ")
                    + '"',
                    texts,
        ...
        if dr_varname[i] in vars_with_duplicate_id_definition_total:
            print(
                (
                    " \n WARNING: A variable is used with an id which is used twice in an id definition. The variable = ",
                    dr_varname[i],
                    " the id = ",
                    var_id_in_created_file_def[i],
                )
            )
        if dr_unit[i] != dr_ping_units[i]:
            print(
                (
                    " \n WARNING: The cmor_unit and ping_unit differ for variable ",
                    dr_varname[i],
                    " units compare as:",
                    dr_unit[i],
                    " versus ",
        ...
        if total_pinglist_field_ref[index_in_ping_list] in [
            "tdenit",
            "tnfix",
            "tcflx",
            "tcflxcum",
            ...
            print(
                (
                    " \n WARNING: The cmor variable",
                    '"' + dr_varname[i] + '"',
                    'with field_ref="'
                    + total_pinglist_field_ref[index_in_ping_list]
                    + '"',
                    'is used with operation="average" while a variable with the same name in the default file_def files uses operation="instant".',
                )
            )
        if total_pinglist_field_ref[index_in_ping_list] in [
            "mldkz5",
            ...
            print(
                (
                    ...
                    + total_pinglist_field_ref[index_in_ping_list]
                    + '"',
                    'is used with operation="average" while a variable with the same name in the default file_def files uses operation="maximum".',
                )
            )
...
if produce_varlistjson_file:
    ...
    command_2 = (
        " drq2varlist --drq "
        + drqlistjson_file_name
        + " --varlist "
        + varlistjson_file_name.replace("all", "all-ec-earth-cc")
        + " --ececonf EC-EARTH-CC "
    )
    command_3 = (
        " drq2varlist --drq "
        + drqlistjson_file_name
        + " --varlist "
        + varlistjson_file_name.replace("all", "all-ec-earth-aogcm")
        + " --ececonf EC-EARTH-AOGCM "
    )
    # os.system(command_1)
    os.system(command_2)
    os.system(command_3)
    ...
    print(
        (
            " \n The produced",
            varlistjson_file_name,
            " is a variant: ordened by model component, the ignored fields are dropped and the preferences are applied.",
        )
    )
...
if os.path.isfile(basic_flat_file_def_file_name) == False:
    print((" The file ", basic_flat_file_def_file_name, "  does not exist."))
    sys.exit(" stop")
...
grid_ref_collection = []
for field in root_basic_file_def.findall(".//field[@component]"):
    component_collection.append(field.attrib["component"])
for field in root_basic_file_def.findall(".//field[@output_freq]"):
    output_freq_collection.append(field.attrib["output_freq"])
for field in root_basic_file_def.findall(".//field[@grid_ref]"):
    grid_ref_collection.append(field.attrib["grid_ref"])
...
for component_value in component_overview:
    ...
    for output_freq_value in output_freq_overview:
        ...
        for grid_ref_value in grid_ref_overview:
            ...
            for field in root_basic_file_def.findall(
                './/field[@component="'
                + component_value
                + '"][@output_freq="'
                + output_freq_value
            ...
            if number_of_fields_per_file != 0:
                ...
                for written_field in root_basic_file_def.findall(
                    './/field[@component="'
                    + component_value
                    + '"][@output_freq="'
                    + output_freq_value
                    ...
                    basic_nemo_file_def_xml_file.write(
                        '     <field id={:37} name={:25} table={:15} field_ref={:40} grid_ref={:32} unit={:20} enabled="False"   operation={:10}   freq_op={:6} > {:70} </field>\n'.format(
                            '"' + written_field.attrib["id"] + '"',
                            '"' + written_field.attrib["name"] + '"',
                            '"' + written_field.attrib["table"] + '"',
                            '"' + written_field.attrib["field_ref"] + '"',
                            '"' + written_field.attrib["grid_ref"] + '"',
                            '"' + written_field.attrib["cmor_table_units"] + '"',
                            '"' + written_field.attrib["operation"] + '"',
                            '"' + written_field.attrib["freq_op"] + '"',
                            written_field.text,
                        )
                    )
                basic_nemo_file_def_xml_file.write("\n    </file>\n")
